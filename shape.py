...
MSG_UPDATE_SHAPE = 'Base Shape not found.|'\
                 ...
                 +  '- Adjust the Tamagoyaki Shape to your mesh (if necessary)\n'\
                 +  '  Or load your original character shape from file\n'\
                 +  '- Attach the Sliders again (Avatar Shape).\n'\
                 +  '\nEXPLAIN:\n'\
                 +  'Tamagoyaki was looking for the reference Shape of your mesh item.\n'\
                 ...
                 +  'This is probably caused by an update from an earlier Tamagoyaki version|'
...
class ShapeCopy(bpy.types.Operator):
    ...
    bl_idname = "tamagoyaki.shape_copy"
    bl_label = "Copy Shape"
...
class ShapePaste(bpy.types.Operator):
    """Paste Shape from internal shape Buffer into active Tamagoyaki Armature"""
    bl_idname = "tamagoyaki.shape_paste"
...
def initialize(rigType):
    log.debug("Loading Tamagoyaki Shape Interface for rigType %s" % (rigType))
    progress=100
    util.progress_begin()
    log.debug("Loading Tamagoyaki DRIVER data rigType %s" % (rigType))
    DRIVERS = data.loadDrivers()
    util.progress_update(progress, False)
    log.debug("Create Tamagoyaki DRIVERS for rigType %s" % (rigType))
    createShapeDrivers(DRIVERS)
    util.progress_update(progress, False)
    log.debug("Loading Tamagoyaki MESH data rigType %s" % (rigType))
    MESHES = data.loadMeshes()
    util.progress_update(progress, False)
    log.debug("Loading Tamagoyaki MESHES for rigType %s" % (rigType))
...
def fromDictionary(obj, dict, update=True, init=False):
    ...
    if update:
        ...
        refreshTamagoyakiShape(context, refresh=True, init=init)
...
def resetToPose(arm_obj, posedata, context, preserveGender, init, force_update):
    ...
    refreshTamagoyakiShape(context)
...
def manage_tamagoyaki_shapes(context, armobj, ids):
    shapes = util.getChildren(armobj, type="MESH")
    meshes = {c.get('mesh_id'):c for c in shapes if 'mesh_id' in c and util.is_tamagoyaki_mesh(c)}
...
def update_on_framechange(scene):
    ...
    try:
        for armobj in [obj for obj in view_layer.objects if obj.type=="ARMATURE" and "tamagoyaki" in obj]:
            if armobj.animation_data and armobj.animation_data.action:
                ...
                try:
                    ...
                    logging.warn("Could not initialise Armature %s (Maybe not an Tamagoyaki ?)", armobj.name)
...
def loadProps(context, obj, filepath, pack=False):
    ...
    refreshTamagoyakiShape(context, refresh=True)
...
def check_dirty_mesh(scene):
    ...
    if active.mode == 'EDIT':
        if armobj and armobj.get('tamagoyaki') and scene.SceneProp.panel_appearance_enabled and not 'tamagoyaki-mesh' in active:
...
def add_shapekey_updater(self, context):
    ...
    if 'tamagoyaki' in armobj and not 'tamagoyaki-mesh' in obj:
...
def destroy_shape_info(context, armobj):
    with util.slider_context() as is_locked:
        objs = util.get_animated_meshes(context, armobj, with_tamagoyaki=False, only_selected=False)
        for ob in objs:
...
class ShapeSliderDetach(bpy.types.Operator):
    ...
    bl_idname = "tamagoyaki.shape_slider_detach"
...
def driven_meshes(context, arm_obj):
    ...
    for obj in animated_meshes:
        target = system_meshes if 'tamagoyaki-mesh' in obj else custom_meshes
        target[obj.name] = obj
...
def shape_slider_driver(self, context, slider_pid):
    ...
    try:
        ...
        omode = util.ensure_mode_is('OBJECT')
        refreshTamagoyakiShape(context, refresh=False, target=slider_pid)
        util.ensure_mode_is(omode)
    finally:
...
def inner_updateShape(context, target, scene, refresh, init, custom_object_map, with_bone_check, only_weights=False):
    ...
    if DIRTY_RIG in armobj:
        ...
        bpy.ops.tamagoyaki.armature_jointpos_store(sync=False)
    ...
    try:
        ...
        if not only_weights:
            ...
            ava_objects = util.getTamagoyakiChildSet(armobj, type='MESH', visible=True)
            log.debug("Updating %d Tamagoyaki meshes and %d custom meshes" % (len(ava_objects),len(custom_object_map)) )
...
def restore_spine_fold_state(armobj):
    ...
    if foldstate == 'none':
        bpy.ops.tamagoyaki.armature_spine_fold()
    if foldstate == 'all':
        bpy.ops.tamagoyaki.armature_spine_unfold()
    if foldstate == 'upper':
