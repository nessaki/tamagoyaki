...
log = logging.getLogger('tamagoyaki.animation')
...
class TAMAGOYAKI_UL_JointOffsetPropVarList(bpy.types.UIList):
    ...
    def draw_item(self,
        ...
        row1.operator('tamagoyaki.focus_on_selected', text="", icon_value=icon_value, emboss=False).bname=key
        row1.operator('tamagoyaki.focus_on_selected', emboss=False, text="%s - (%s)" % (key, marker)).bname=key
        if factor <1:
...
def setReference(context, src_armobj, tgt_armobj, target_bone_info, frame=None, target_root_bone_name='COG'):
    ...
    root_offset = getCenterBoneDistance(context, src_armobj, tgt_armobj, frame=frame, target_root_bone_name=target_root_bone_name)
    log.warning("setReference(): Goffset: %s " % root_offset)

    for channel in target_bone_info.values():

        channel.Goffset = root_offset
...
def transferActionData(context, source, target, target_bone_info, reference_frame, start_frame, end_frame, keep_reference_frame, target_root_bone_name, progress):
    ...
    root_offset = getCenterBoneDistance(context, source, target, frame=reference_frame, target_root_bone_name=target_root_bone_name)
    ...
    for frame in range(start_frame, end_frame+1):
        ...
        context.scene.frame_set(frame)
        rig.retarget_armature_from_mocap(context, frame, depsgraph, target, mocap, False, source_ref_pose, target_ref_pose, root_offset)
        if not counter % 20:
            log.warning("Processed %d/%d frames" % (counter, 1 + end_frame - start_frame))
...
def transferAction(context, source, target, target_bone_info, reference_frame, start_frame, end_frame, 
                   mocap, 
                   with_translation, 
                   keep_reference_frame,
                   target_root_bone_name):
    ...
    collectMotionData(context, source, target_bone_info, reference_frame, start_frame, end_frame)
...
class AvatarAnimationTrimOp(bpy.types.Operator):
    bl_idname = "tamagoyaki.action_trim"
...
class ImportAvatarAnimationOp(bpy.types.Operator, ImportHelper):
    bl_idname = "tamagoyaki.import_avatar_animation"
    bl_label = "SL Animation (bvh)"
    bl_description ="Create new default Tamagoyaki Character and import bvh"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def draw(self, context):
        ...
        if armobj:
            ...
            row.prop(mocap,"rigtype",expand=True)

        col = layout.column()
        col.prop(mocap, 'target_root_bone_name', text='')
        col.prop(mocap, 'with_translation')
        col.prop(mocap, 'global_scale')
        col.prop(mocap, 'with_sanitize')
        col.prop(mocap, 'keep_source_rig')
        col.prop(mocap, 'keep_reference_frame')

        if util.get_ui_level() > UI_ADVANCED:
            col.prop(mocap,'use_restpose')
            col = layout.column()
            col.prop(mocap,'reference_frame')
            col.enabled= not mocap.use_restpose
    ...
    def exec_trans(
        ...
        try:
            ...
            if reference_frame != None:
                mocap.reference_frame = reference_frame

            ButtonTransfereMotion.transferMotion(context, keep_reference_frame, target_root_bone_name)
            if with_translation:
                set_restpose(target)

            if with_sanitize and target.animation_data and target.animation_data.action:
    ...
    def execute(self, context):
        ...
        try:
            for i in self.files:
                ...
                try:
                    log.warning("use Restpose     : %s" % mocap.use_restpose)
                    log.warning("reference frame  : %d" % mocap.reference_frame)
                    log.warning("with Translation : %s" % mocap.with_translation)
                    log.warning("reference root   : %s" % mocap.target_root_bone_name)
                    log.warning("with Sanitize    : %s" % mocap.with_sanitize)
                    log.warning("seamlessRotFrames: %d" % mocap.seamlessRotFrames)
                    log.warning("seamlessLocFrames: %d" % mocap.seamlessRotFrames)
                    ImportAvatarAnimationOp.exec_imp(
                        context,
                        armobj,
                        filepath,
                        mocap.use_restpose,
                        mocap.reference_frame,
                        mocap.target_root_bone_name,
                        mocap.with_translation,
                        mocap.with_sanitize,
                        mocap.global_scale,
                        mocap.keep_source_rig,
                        mocap.keep_reference_frame
                        )
...
class ButtonTransfereMotion(bpy.types.Operator):
    bl_idname = "tamagoyaki.transfer_motion"
...
- The COG Bone is not mapped to a bone in the Source Rig'''
    ...
    def prepare_target_for_animation_transfer(context, target):
        ...
        unlock_list = ["HandThumb0Left", "HandThumb0Right", "HandThumb1Left", "HandThumb1Right", "HandThumb2Left", "HandThumb2Right", "HandThumb3Left", "HandThumb3Right", "HandThumb4Left", "HandThumb4Right"]

        def change_parent(armobj, bname, pname):
            b = target.data.edit_bones.get(bname)
            p = target.data.edit_bones.get(pname)
            if b and p:
                b.parent=p

        def unlock(target, unlock_list):
            for name in unlock_list:
                b = target.pose.bones.get(name)
                if b:
        ...
        def hide(target, bname):
            target.data.bones[bname].hide=True

        omode = util.set_object_mode('EDIT')
        change_parent(target, "HandThumb0Right", "WristRight")
        change_parent(target, "HandThumb0Left", "WristLeft")

        util.set_object_mode('POSE')
        hide(target, 'ThumbControllerLeft')
        hide(target, 'ThumbControllerRight')
        unlock(target, unlock_list)
        util.set_object_mode(omode)

    @staticmethod
    def transferPoses(context, reference_frame, start_frame, end_frame, keep_reference_frame, target_root_bone_name):
        scn = context.scene
        mocap = scn.MocapProp
        source = bpy.data.objects[mocap.source]
        target = bpy.data.objects[mocap.target]

        ButtonTransfereMotion.prepare_target_for_animation_transfer(context, target)

        show_bone_mapper = mocap.show_bone_mapper
        if show_bone_mapper:
            mocap.show_bone_mapper = False
        target_bone_info = get_target_bone_info(mocap, with_translation=True)
        setReference(context, source, target, target_bone_info, 
                     reference_frame,
                     target_root_bone_name=target_root_bone_name )
        transferAction(context, source, target, target_bone_info,
                       reference_frame,
                       ...
                       target_root_bone_name)
        if show_bone_mapper:
            mocap.show_bone_mapper = True

    def execute(self, context):
        osupp = util.set_disable_handlers(context.scene, True)
        try:
            target_root_bone_name = context.scene.MocapProp.target_root_bone_name
            ButtonTransfereMotion.transferMotion(context, self.keep_reference_frame, target_root_bone_name)
        except Exception as e:
    ...
    @staticmethod
    def transferMotion(context, keep_reference_frame, target_root_bone_name):
        scn = context.scene
        mocap = scn.MocapProp
        ...
        start_frame, end_frame = ButtonCleanupTarget.get_retarget_frame_range(context)
        if start_frame > end_frame:
            return

        ButtonCleanupTarget.delete_action_data(context, target, start_frame, end_frame, reference_frame) # to get rid of a previous transfer on the same action
        ButtonTransfereMotion.transferPoses(context, reference_frame, start_frame, end_frame, keep_reference_frame, target_root_bone_name)

        util.set_active_object(context, oobj)
        util.set_object_mode(omode)
...
class ButtonTransferePose(bpy.types.Operator):
    ...
    def set_bone_select_mode(rig_name, status):
        rig = bpy.data.objects.get(rig_name)
        if rig:
            util.set_bone_select_mode(rig, status)


    def prepare(context, mocap):
        target = bpy.data.objects.get(mocap.target)
        ...
        util.select_single_object(target)
        util.set_object_mode('POSE')
        ButtonTransferePose.set_bone_select_mode(mocap.target, True)
        bpy.ops.pose.transforms_clear()
        ...
        ButtonTransferePose.set_bone_select_mode(mocap.target, False)
        return omode, oobj, lomode, target

    def restore(context, omode, oobj, lomode):
        util.set_object_mode(lomode)
        util.set_active_object(context, oobj)
        util.set_object_mode(omode)
    ...
    def execute(self, context):
        ...
        try:
            ...
            omode, oobj, lomode, target = ButtonTransferePose.prepare(context, mocap)

            depsgraph = context.evaluated_depsgraph_get()
            depsgraph.update()
            ButtonTransfereMotion.transferPoses(context, reference_frame, current_frame, current_frame, True, mocap.target_root_bone_name)
            ButtonTransferePose.restore(context, omode, oobj, lomode)
...
class ButtonMatchScales(bpy.types.Operator):
    bl_idname = "tamagoyaki.match_scales"
    bl_label ="Match scales"
    bl_description ="Match the scale of the imported armature to the Tamagoyaki"
    bl_options = {'REGISTER', 'UNDO'}
...
class ButtonCleanupTarget(bpy.types.Operator):
    bl_idname = "tamagoyaki.delete_motion"
    bl_label ="Delete Motion"
    bl_description ="Delete motion from Timeline"
    bl_options = {'REGISTER', 'UNDO'}

    def get_retarget_frame_range(context):
        scene = context.scene
        mocap = scene.MocapProp
        source = bpy.data.objects[mocap.source]
        target = bpy.data.objects[mocap.target]

        source_start_frame, source_end_frame = [int(f) for f in source.animation_data.action.frame_range]
        scene_start_frame = scene.frame_start
        scene_end_frame   = scene.frame_end

        if mocap.use_scene_action_range:
            start_frame = max(source_start_frame, scene_start_frame)
            end_frame   = min(source_end_frame, scene_end_frame)
        else:
    ...
    def delete_action_data(context, target, start_frame, end_frame, reference_frame):
        scn = context.scene
    ...
    def execute(self, context):
        ...
        try:
            ...
            ButtonCleanupTarget.delete_action_data(context, target, start_frame, end_frame, reference_frame)
...
class KeyPoseChanges(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        def resolve_path(arm, fcurve):

            if "PelvisInv" in fcurve.data_path and "Tinker" in arm.data.bones:
            ...
            try:
                ...
                log.warn("Fcurve path [%s] can not be resolved (ignore bone)" % bone_path)
            return bone, ctype
        ...
        if arm.animation_data.action:
            ...
            for fcurve in fcurves:
                bone, ctype = resolve_path(arm, fcurve)
                if not bone:
        ...
        for bone in arm.pose.bones:
            ...
            if not util.bone_is_visible(arm, dbone):
                continue
            if bone.parent == None:
                continue # Do not touch the skeleton root bone (works for Tamagoyaki)
...
class ButtonCopyTimelineRange(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        visual_keying_enabled = context.preferences.edit.use_visual_keying
        context.preferences.edit.use_visual_keying = False
