...
### Modifications 2021      Machinimatrix
...
log = logging.getLogger('tamagoyaki.main')
updatelog = logging.getLogger('tamagoyaki.update')
visitlog = logging.getLogger('tamagoyaki.visit')
...
def copypaste_pose(self, context):
    ...
    if ob and ob.type=='ARMATURE' and ob.mode=='POSE':
        ...
        self.layout.operator("tamagoyaki.pose_copy", text='', icon='COPYDOWN')
        self.layout.operator("tamagoyaki.pose_paste", text='', icon='PASTEDOWN')
        self.layout.operator("tamagoyaki.pose_mirror_paste", text='', icon="PASTEFLIPDOWN")
        self.layout.operator("tamagoyaki.key_pose_changes", text='', icon=ICON_KEY_HLT)

class PoseCopy(bpy.types.Operator):
    bl_idname = "tamagoyaki.pose_copy"
    bl_label  = "Copy Pose"
...
class PosePaste(bpy.types.Operator):
    bl_idname = "tamagoyaki.pose_paste"
    bl_label  = "Paste Pose"
...
class PoseMirrorPaste(bpy.types.Operator):
    bl_idname = "tamagoyaki.pose_mirror_paste"
...
def selectedAddonCallback(self, context):
    ...
    if preferences.productName   == 'Tamagoyaki':
...
class Tamagoyaki(AddonPreferences):
    ...
    if not os.path.exists(config_file):
        ...
        if not os.path.exists(CONFIG_DIR):
            print("Create Tamagoyaki configuration folder")
    ...
    ui_complexity  : EnumProperty(
        items=(
            ('0', 'Basic', 'Show the most basic Tamagoyaki functions'),
            ('1', 'Advanced', 'Show the most useful Tamagoyaki functions'),
    ...
    use_asset_library_link : BoolProperty(default=False, name="Link Assets",
        description = "When this option is enabled,\n"
                    + "then the textures and meshes from Tamagoyaki's\n"
    ...
    always_alter_restpose : BoolProperty(
        ...
        default=False,
        description="Tamagoyaki binds to the T-Pose by default (recommended).\n"\
                    +"Setting this option enforces a Bind to the visual pose.\n"\
                    +"Note: Bind to visual pose disables the Tamagoyaki shape\n"\
    ...
    show_panel_fitting : BoolProperty(
        default=False,
        name="Enable Fitting Panel",
        description=\
'''The fitting panel is only used by very few people,
mostly by full avatar body makers. Since the fitting panel
is not easy to use, we hide it by default, so that users
do not accidentally confuse this panel with the Appearance panel'''
        )
    ...
    enable_auto_rig_update : BoolProperty(
        ...
        description="recalculate Avatar Shape Parameters.\n"\
                    +"Detail:\n"\
                    +"Tamagoyaki must adjust Avatar shape to the Rig\n"\
    ...
    enable_auto_mesh_update : BoolProperty(
        ...
        description="recalculate reference Mesh for dirty Rig.\n"\
                    +"Detail:\n"\
                    +"Tamagoyaki must adjust Reference Meshes to the sliders\n"\
    ...
    def draw_create_panel(self, context, box):
        ...
        if last_select not in ["Rig Presets", "Presets"]:
            ...
            row.operator("tamagoyaki.rig_presets_remove", text="", icon=ICON_REMOVE).remove_active = True

        col = box.column(align=True)
        row = col.row(align=True)
        row.label(text='tamagoyakiMeshType')#text=propgroups.ScenePropGroup.tamagoyakiMeshType[1]['name'])
        row.prop(sceneProps, "tamagoyakiMeshType",   text='')

        row = col.row(align=True)
        row.label(text='tamagoyakiRigType')#text=propgroups.ScenePropGroup.tamagoyakiRigType[1]['name'])
        row.prop(sceneProps, "tamagoyakiRigType",   text='')
    ...
    def draw(self, context):
        ...
        def draw_link_section(box):
            col = box.column(align=True)
            col.operator("wm.url_open", text="Tamagoyaki Release Info ...", icon=ICON_URL).url=RELEASE_INFO
        ...
        def draw_devkit_section(box):
            ...
            row=col.row(align=True)
            row.operator("tamagoyaki.import_devkit_configuration", text = "Import Preset(s)", icon=ICON_IMPORT)
            row.prop(kitprop, "devkit_replace_import", toggle=True, text='', icon=ICON_DECORATE_OVERRIDE)

            row=col.row(align=True)
            oprop = row.operator("tamagoyaki.export_devkit_configuration", text='Export %s Config'%preset_name, icon = ICON_EXPORT)
        ...
        def draw_devkit_editor(layout, kitprop):
            ...
            tcol = tbox.column()
            tcol.label(text="Tamagoyaki Config:")
            tcol.prop(kitprop, "tgtRigType",   text="Rig")
            tcol.prop(kitprop, "tgtJointType", text="Joint")
            tcol.prop(kitprop, "tgtSkeletonType", text="Skeleton")
            ...
            row = col.row(align=True)
            prop = row.operator("tamagoyaki.devkit_presets_add", text="Add/Replace Devkit Preset")
            row.operator("tamagoyaki.devkit_manager_cut_preset", text='', icon=ICON_FREEZE)
        ...
        prop = col.operator("tamagoyaki.rig_presets_reset", text="Reset to Factory Setting")
        prop.category = "rigs"
        ...
        util.ErrorDialog.draw_generic(
            help_box,
            messages.panel_info_devkit_presets%DOCUMENTATION,
            SEVERITY_HINT,
...
class TamagoyakiShowPrefs(Operator):
    bl_idname = "tamagoyaki.pref_show"
    bl_description = 'Open Tamagoyaki addon preferences for customization'
    bl_label = "Tamagoyaki Preferences"
    bl_options = {'INTERNAL'}
    ...
    def execute(self, context):
        ...
        if mod is None:
            print("Tamagoyaki seems to be not enabled or broken in some way")
            return {'CANCELLED'}
...
class DevkitConfigurationEditor (util.ExpandablePannelSection):
    bl_idname = "tamagoyaki.devkit_configuration_editor"
    bl_label = "Create/Modify Configuration"


class DownloadReload(bpy.types.Operator):
    bl_idname = "tamagoyaki.download_reload"
...
class DownloadInstall(bpy.types.Operator):
    bl_idname = "tamagoyaki.download_install"
    bl_label  = "Install Download"
    bl_description = "Install the just downloaded Tamagoyaki Update"
...
class DownloadReset(bpy.types.Operator):
    bl_idname = "tamagoyaki.download_reset"
...
class CheckForUpdates(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        dld = None
        try:
            dld=service.tamagoyaki.getPlugin(1,user, pwd, addon_version, blender_version, product)
...
''' class PanelTamagoyakiInfo(bpy.types.Panel):
...
class TamagoyakiAddPresetRig(AddPresetBase, Operator):
...
class TamagoyakiUpdatePresetRig(AddPresetBase, Operator):
...
class TamagoyakiRemovePresetRig(AddPresetBase, Operator):
...
class ObjectSelectOperator(bpy.types.Operator):
    bl_idname      = "tamagoyaki.object_select_operator"
    bl_label       = "select"
...
class DisplayTamagoyakiVersionOperator(bpy.types.Operator):
...
Notes
- The Rig ID can be the same over several Tamagoyaki releases.
...
class DisplayTamagoyakiVersionMismatchOperator(bpy.types.Operator):
    bl_idname      = "tamagoyaki.version_mismatch"
    bl_label       = "Version Mismatch"
    bl_description = "You probably use a rig from an older Tamagoyaki. Click for details"
    msg = 'Version Mismatch|'\
        +  'YOUR ACTION:\n'\
        +  'Upgrade your rig to the current Tamagoyaki version.|%s|Rig Migrate ...|'\
        +  '\nDETAIL:\n'\
        +  'Your Rig has been made with a different version of Tamagoyaki.\n\n'\
...
class DisplayTamagoyakiRigVersionOperator(bpy.types.Operator):
...
class WeightAcceptedHint(bpy.types.Operator):
    bl_idname      = "tamagoyaki.weight_accept_hint"
    bl_label       = "accepted"
...
class WeightIgnoredHint(bpy.types.Operator):
    bl_idname      = "tamagoyaki.weight_ignore_hint"
    bl_label       = "ignored"
...
class ShapeTypeMorphHint(bpy.types.Operator):
    bl_idname      = "tamagoyaki.shape_type_morph_hint"
    bl_label       = "Shape Type"
...
class ShapeTypeSystemMorphHint(bpy.types.Operator):
    bl_idname      = "tamagoyaki.shape_type_system_morph_hint"
    bl_label       = "Shape Type"
    bl_description = "This is a Morph slider\n"\
...
class ShapeTypeBoneHint(bpy.types.Operator):
    bl_idname      = "tamagoyaki.shape_type_bone_hint"
    bl_label       = "Shape Type"
...
class ShapeTypeExtendedHint(bpy.types.Operator):
    bl_idname      = "tamagoyaki.shape_type_extended_hint"
    bl_label       = "Shape Type"
...
class ShapeTypeFittedHint(bpy.types.Operator):
    bl_idname      = "tamagoyaki.shape_type_fitted_hint"
    bl_label       = "Shape Type"
...
class FittingTypeHint(bpy.types.Operator):
    bl_idname      = "tamagoyaki.fitting_type_hint"
...
class FittingBoneSelectedHint(bpy.types.Operator):
    bl_idname      = "tamagoyaki.fitting_bone_selected_hint"
...
class ResetShapeSectionOperator(bpy.types.Operator):
    bl_idname      = "tamagoyaki.reset_shape_section"
...
class ResetShapeValueOperator(bpy.types.Operator):
    bl_idname      = "tamagoyaki.reset_shape_slider"
    ...
    def execute(self, context):
        ...
        try:
            ...
            shape.refreshTamagoyakiShape(context, refresh=True)
            util.enforce_armature_update(context, armobj)
        finally:
            shape.recurse_call = False
            util.set_active_object(context, active)
...
class ButtonLoadShapeUI(bpy.types.Operator):
    bl_idname = "tamagoyaki.load_shape_ui"
    bl_label = "Load Avatar Shape Editor"
    bl_description = "Load the Tamagoyaki Shape User Interface (Avatar Shape)"
...
class ButtonPrintProps(bpy.types.Operator):
    ...
    '''
    bl_idname = "tamagoyaki.print_props"
...
class ButtonSaveProps(bpy.types.Operator):
    ...
    import a shape into OpenSim (or similar online worlds) other than manually setting the values
    '''
    bl_idname = "tamagoyaki.save_props"
...
class ButtonLoadProps(bpy.types.Operator):
    ...
    '''
    bl_idname = "tamagoyaki.load_props"
...
class ButtonResetToSLRestPose(bpy.types.Operator):
    ...
    '''
    bl_idname = "tamagoyaki.reset_to_restpose"
...
class ButtonResetToDefault(bpy.types.Operator):
    ...
    '''
    bl_idname = "tamagoyaki.reset_to_default"
...
class ButtonResetToBindshape(bpy.types.Operator):
    ...
    '''
    bl_idname = "tamagoyaki.reset_to_bindshape"
    bl_label = "Bind Shape"
    bl_description = \
...
class ButtonDeleteAllShapes(bpy.types.Operator):
    '''
    Delete Tamagoyaki default shapes.
    '''
    bl_idname = "tamagoyaki.manage_all_shapes"
    bl_label ="Manage Tamagoyaki Meshes"
    ...
    def has_id(self, arm, key):
        ...
        for child in (child for child in shapes if util.is_tamagoyaki_mesh(child)):
    ...
    def execute(self, context):
        try:
            ...
            shape.manage_tamagoyaki_shapes(context, armobj, ids)
        except Exception as e:
            util.ErrorDialog.exception(e)
        return {'FINISHED'}
...
class ButtonRefreshShape(bpy.types.Operator):
    ...
    '''
    bl_idname = "tamagoyaki.refresh_character_shape"
...
class ButtonIKMatchDetails(bpy.types.Operator):
    bl_idname = "tamagoyaki.ikmatch_display_details"
    bl_label = ""
...
class ButtonIKMatchAll(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_match_all"
    ...
    def poll(self, context):
        if context == None:
            msg = "tamagoyaki.ik_match_all: No context available while polling"
            return False

        ob = context.object
        if ob == None:
            msg = "tamagoyaki.ik_match_all: No context object available while polling"
            return False

        if ob.mode != 'POSE':
            msg = "tamagoyaki.ik_match_all: Context object [%s] is in[%s] mode (where POSE was needed)" % (ob.name, ob.mode)
            return False

        try:
            if "tamagoyaki" in context.active_object:
                return True
        except TypeError:
            ...
            if active.IKSwitchesProp.Show_All or not bones.isdisjoint(LArmBones):
                try:
                    ...
                    if con:
                        ...
                        props = row.operator("tamagoyaki.ik_apply", text='', icon=ICON_POSE_DATA)
                        props.limb='ARM'
                        props.symmetry='Left'
...
class ButtonEnableEyeTarget(bpy.types.Operator):
    bl_idname = "tamagoyaki.eye_target_enable"
...
class ButtonIKWristLOrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_wrist_l_orient"
    bl_label ="Match Left"
...
class ButtonIKElbowTargetLOrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_elbowtarget_l_orient"
    bl_label ="Set Target"
...
class ButtonIKWristROrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_wrist_r_orient"
...
class ButtonIKElbowTargetROrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_elbowtarget_r_orient"
    bl_label ="Set Target"
...
class ButtonIKHeelLOrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_heel_l_orient"
    bl_label ="Match Left"
...
class ButtonIKKneeTargetLOrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_kneetarget_l_orient"
    bl_label ="Set Target"
...
class ButtonIKHeelROrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_heel_r_orient"
    bl_label ="Match Right"
...
class ButtonIKKneeTargetROrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_kneetarget_r_orient"
    bl_label ="Set Target"
...
class ButtonIKHindLimb3LOrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_hindlimb3_l_orient"
    bl_label ="Match Left"
...
class ButtonIKHindLimb2TargetLOrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_hindlimb2target_l_orient"
    bl_label ="Set Target"
...
class ButtonIKHindLimb3ROrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_hindlimb3_r_orient"
    bl_label ="Match Right"
...
class ButtonIKHindLimb2TargetROrient(bpy.types.Operator):
    bl_idname = "tamagoyaki.ik_hindlimb2target_r_orient"
    bl_label ="Set Target"
...
class ButtonChainMore(bpy.types.Operator):
    bl_idname = "tamagoyaki.chain_more"
    bl_label ="More"
...
class ButtonChainLess(bpy.types.Operator):
    bl_idname = "tamagoyaki.chain_less"
    bl_label ="Less"
...
class ButtonChainCOG(bpy.types.Operator):
    bl_idname = "tamagoyaki.chain_cog"
    bl_label = "COG"
...
class ButtonChainParent(bpy.types.Operator):
    bl_idname = "tamagoyaki.chain_parent"
    bl_label ="Parent"
...
class ButtonChainLimb(bpy.types.Operator):
    bl_idname = "tamagoyaki.chain_limb"
    bl_label ="Limb"
...
class ButtonChainFree(bpy.types.Operator):
    bl_idname = "tamagoyaki.chain_free"
    bl_label ="Free"
...
class ButtonChainClamped(bpy.types.Operator):
    bl_idname = "tamagoyaki.chain_clamped"
    bl_label ="Clamped"
...
class ButtonSetRotationLimits(bpy.types.Operator):
    bl_idname = "tamagoyaki.set_rotation_limits"
    bl_label ="Set"
...
class ButtonUnsetRotationLimits(bpy.types.Operator):
    bl_idname = "tamagoyaki.unset_rotation_limits"
    bl_label ="Unset"
...
class ButtonBreatheIn(bpy.types.Operator):
    ...
    '''
    bl_idname = "tamagoyaki.breathe_in"
    bl_label ="In"
...
class ButtonBreatheOut(bpy.types.Operator):
    ...
    (movement is subtle - hit repeatedly for stronger effect)
    '''
    bl_idname = "tamagoyaki.breathe_out"
    bl_label ="Out"
...
class ButtonCustomShape(bpy.types.Operator):
    bl_idname = "tamagoyaki.use_custom_shapes"
...
class ButtonStickShape(bpy.types.Operator):
    bl_idname = "tamagoyaki.use_stick_shapes"
...
class PanelAnimationExport(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        seconds = (endprop.frame_end-startprop.frame_start)/fpsprop.fps

        col.enabled = not is_bulk_export
        col.alert = seconds > 60
        ...
        col = col.column(align=True)
        col.alert = seconds > 60
        col.label(text="Runtime: %03.1f Seconds" % seconds)
        ...
        if props.Mode == 'anim':
            ...
            col.prop(props,"Simplify_Factor", text="Compression")
...
class ExportAnimOperator(bpy.types.Operator):
    ...
    '''
    bl_idname = "tamagoyaki.export_anim"
...
Note: The .anim format is the SL internal format'''
    ...
    def get_export_name(armobj):
        if armobj == None:
            raise ("Need Armature to get Export Name")
        ...
        if action:
            ...
            action_fps = animProps.fps
            frame_start = animProps.frame_start
            frame_end = animProps.frame_end
        else:
            if len(animation_data.nla_tracks)==0:
                return "", ""
            actionname = "%s-NLA" % armobj.name
            animProps = armobj.AnimProp
            action_fps = bpy.context.scene.render.fps
        ...
        runtime = int(round((frame_end-frame_start) / action_fps))
    ...
    def invoke(self, context, event):
        log.warning("Invoke tamagoyaki.export_anim...")
        obj       = context.active_object
        armobj    = util.get_armature(obj)
    ...
    def execute(self, context):
        ...
        log.warning("Execute tamagoyaki.export_anim...")
        active = context.active_object
        amode = active.mode
        armobj = util.get_armature(active)
        export_counter = 0
...
class ButtonExportAnim(ExportAnimOperator):
    ...
    Export the animation
    '''
    bl_idname = "tamagoyaki.export_single_anim"
    bl_label ="Export Animation"
    bl_description = \
...
class ButtonExportBulkAnim(ExportAnimOperator):
    ...
    Export the animation
    '''
    bl_idname = "tamagoyaki.export_bulk_anim"
...
def fix_tamagoyaki_data_on_load(dummy):
    ...
    if props.fix_data_on_upload:
        log.debug("handler [%s] started" % "fix_tamagoyaki_data_on_load")
    else:
        log.debug("Automatic fix of Tamagoyaki data after load is disabled.")
        return
    ...
    def fix_armature_data(context, armobj, props):
        ...
        if not armobj.library:
            ...
            rig.fix_tamagoyaki_armature(context, armobj)
                ...
                for window in context.window_manager.windows:
                    ...
                    for area in screen.areas:
                        if area.type == 'VIEW_3D':
                            ...
                            break
                if ctx:
                    bpy.ops.tamagoyaki.update_tamagoyaki(ctx, 'INVOKE_DEFAULT')
    ...
    def copy_RigProps_to_RigProp(armobj):
        ...
        if rigProps:
            ...
            rigProp.SkeletonType = rigProp.SkeletonTypeItems[get_prop_safemode(rigProps, 'SkeletonType', default=0)][0]
    ...
    try:
        ...
        util.set_disable_handlers(scene, False)
...
class PanelRetargetInfo(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.label(text='- An Tamagoyaki Skeleton')
        col.label(text='- The Tamagoyaki is selected')
        col.separator()
        bbox=box.box()
...
class PanelPoseTransfer(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        mocap_sources = [obj.name for obj in bpy.data.objects if 'tamagoyaki' not in obj and obj.type == 'ARMATURE']
        mocap_targets = [obj.name for obj in bpy.data.objects if 'tamagoyaki' in obj and obj.type == 'ARMATURE']
        ...
        if prop.target in  mocap_targets and prop.source in  mocap_sources:
            ...
            row.prop(prop, "reference_frame", text="")
            row.enabled = not prop.use_restpose

            col = box.column(align=True)
            col.prop(prop, "target_root_bone_name", text='')
            col.prop(prop, "use_restpose")
            col.prop(prop, "with_translation")

            col = box.column(align=True)
            row = col.row(align=True)
            row.operator("tamagoyaki.transfer_pose", text='Transfer Pose', icon=ICON_OUTLINER_OB_ARMATURE)
            row.operator("tamagoyaki.match_scales", text='Match scales', icon=ICON_SOLO_ON)

            map_counter = get_retarget_map_counter(prop, target, exact_count=False)
...
selected_target_bones = []
selected_source_bones = []
...
first selected bone in the source rig

NOTE: You can enable AUTO accept mode to quickly map many bone pairs within seconds
'''

    def execute(self, context):
        global selected_target_bones
        global selected_source_bones
        prop = bpy.context.scene.MocapProp
        if prop.source\
           and prop.target\
           and len(selected_target_bones) > 0\
           and len(selected_source_bones) > 0:

            for target_bone_name in selected_target_bones:
                prop[target_bone_name] = selected_source_bones[0]
        adjust_retarget_mapping(prop)
        return {'FINISHED'}
...
class PanelMotionTransfer(bpy.types.Panel):
    ...
    def show_interactive_mapper(self, context, box, target, selected_source_bones, selected_target_bones):
        ...
        source_bone_col = row.column()
        target_bone_col = row.column()
        source_bone_col.label(text="Source bone")
        target_bone_col.label(text="Target bone(s)")
        for b in selected_source_bones:
            source_bone_col.label(text=b)
        for b in selected_target_bones:
            target_bone_col.label(text=b)

        can_match = True
        col = box.column()
        if len(selected_source_bones) > 1:
            col.alert=True
            col.label(text="To many source bones selected")
            can_match=False
        elif len(selected_source_bones) == 0:
            source_bone_col.alert=True
            source_bone_col.label(text="Select a Source bone!")
            can_match=False
        if len(selected_target_bones) == 0:
            target_bone_col.alert=True
            target_bone_col.label(text="Select a Target bone!")
            can_match=False
        elif len(selected_target_bones) >1:
            col.label(text="Note: Mapping to %d bones" % len(selected_target_bones))
    ...
    def draw(self, context):
        ...
        if prop.target in  mocap_targets and prop.source in  mocap_sources:
            ...
            if prop.show_bone_mapping:
                ...
                mapping_icon = ICON_DISCLOSURE_TRI_RIGHT

            if prop.show_bone_mapper:
                mapper_icon = ICON_DISCLOSURE_TRI_DOWN
            else:
                mapper_icon = ICON_DISCLOSURE_TRI_RIGHT
            ...
            if prop.show_bone_mapping:
                ...
                if active_pose_bone:
                    marked_bone_name = active_pose_bone.name if active_pose_bone.name in selected_target_bones else None
                else:
                ...
                map_counter = get_retarget_map_counter(prop, target, exact_count=False)
                gcol = layout.column()
                gcol.alert = map_counter == 0
                if map_counter == 0:
                    gcol.operator(ButtonGuessMapping.bl_idname, icon=ICON_ERROR)
                else:
                    gcol.operator(ButtonGuessMapping.bl_idname, icon=ICON_MONKEY)
                ...
                if prop.show_bone_mapper:
                    ...
                    row.operator(ButtonMappingDisplayMapper.bl_idname, text="Interactive Bone Mapper", icon=mapper_icon)

                    if source.mode=='POSE' and target.mode=='POSE':
                        pass
                    else:
                        bbox.alert=True
                        col=bbox.column(align=True)
                    ...
                    col = box.column(align=True)
                    col.operator(ButtonMappingDisplayMapper.bl_idname, text="Interactive Bone Mapper", icon=mapper_icon)
                ...
                markercol = row.column(align=True)
                sourcecol = row.column(align=True)

                targetcol.scale_x = 1.3
                sourcecol.scale_x = 1.5
                markercol.scale_x = 0.5
                selectcol = row.column(align=True)
                selectcol.scale_x = 0.5
                ...
                visible_bone_names = util.getVisibleBoneNames(target)
                for bone in data.get_mtui_bones(target):
                    ...
                    if not bone in visible_bone_names:
                        continue

                    display_counter += 1
                    reset_counter += 1
                    targetcol.label(text=bone+":")
                    marker = ICON_PIVOT_CURSOR if bone in selected_target_bones else ICON_BLANK1
                    row = markercol.row(align=True)
                    row.label(icon=marker)

                    sourcecol.prop_search(prop, bone, source.data, "bones", text='')
                    row=selectcol.row(align=True)
                    row.operator(ButtonSetSourceBone.bl_idname, text='', icon=ICON_PIVOT_CURSOR).target_bone = bone
                    row.label(text='')
                ...
                if map_counter > 0:
                    ...
                    if display_counter == 0:
                        col = box.column()
                        col.alert=True
                        col.label(text="All mapped bones are hidden")
                else:
                    col = box.column()
                    col.alert=True
                ...
                for bone in data.get_mtui_bones(target):
                    ...
                    if link and  link != '':
                        map_counter +=1
                display_counter = map_counter


            gcol = mcol.column()
            mcol.alert = map_counter == 0
            mrow = mcol.row(align=True)
            mrow.operator(ButtonMappingDisplayDetails.bl_idname, text="Mapping Table", icon=mapping_icon)

                
def update_source_rig(source, target, selection):
    prop = bpy.context.scene.MocapProp
    for bone_name in selection:
        bone = source.pose.bones.get(bone_name)
        if bone:
            bone.bone.select=True

def update_target_rig(source, target, selection):
    prop = bpy.context.scene.MocapProp
    for bone_name in selection:
        bone = target.pose.bones.get(bone_name)
        if bone:
            bone.bone.select=True
            create.mark_visual_retarget_bone_group(target, prop, bone)
            link = prop.get(bone_name)
            if link:
                bone.bone_group = create.get_bone_group(target, 'active')
                bone = source.pose.bones.get(link)
                bone.bone_group = create.get_bone_group(source, 'active')
                bone.bone.select=True


def adjust_retarget_mapping(prop):
    source = bpy.data.objects.get(prop.source)
    target = bpy.data.objects.get(prop.target)

    unmapped_source_group = source.pose.bone_groups.get('unmapped')
    active_source_group = source.pose.bone_groups.get('active')

    for source_bone in source.pose.bones:
        if source_bone.bone_group != unmapped_source_group and not source_bone.name in prop.values():
            source_bone.bone_group = unmapped_source_group
    for target_bone_name in data.get_mtui_bones(target):
        target_bone = target.pose.bones.get(target_bone_name)
        link = prop.get(target_bone_name)
        if link:                                
            source_bone = source.pose.bones.get(link)
            if target_bone.bone.select:
                source_bone.bone_group = create.get_bone_group(source, 'active')
                target_bone.bone_group = create.get_bone_group(target, 'active')
            else:
                original_bone_group_name = target_bone.get('original_bone_group_name')
                if original_bone_group_name:
                    source_bone.bone_group = create.get_bone_group(source, original_bone_group_name)
                    target_bone.bone_group = create.get_bone_group(target, original_bone_group_name)
                else:
                    target_bone['original_bone_group_name'] = target_bone.bone_group.name if target_bone.bone_group else None
        else:
            original_bone_group_name = target_bone.get('original_bone_group_name')
            if not original_bone_group_name:
                target_bone['original_bone_group_name'] = target_bone.bone_group.name if target_bone.bone_group else None
            target_bone.bone_group = create.get_bone_group(target, 'unmapped')

    util.ensure_mode_is('POSE', object=source)
    util.ensure_mode_is('POSE', object=target)
...
def check_active_bone_changed(scene, depsgraph, forced_check=False):
    global selected_target_bones
    global selected_source_bones
    global block_retarget_bone_group_processing

    if not forced_check:
        if not scene.MocapProp.show_bone_mapper:
            return
        if block_retarget_bone_group_processing:
            return
    ...
    def selection_changed(old_selection, new_selection):
        return set(old_selection) != set(new_selection)

    def clear_rig(rig, selection):
        for bone_name in selection:
    ...
    def update_rig(rig, bone, mark_visual=False):
        bone.bone.select=True
        if mark_visual:
            prop = bpy.context.scene.MocapProp
            create.mark_visual_retarget_bone_group(rig, prop, bone)


    def prepare_source_bone_groups(source, target):
        oactive = util.set_active_object(bpy.context, target)
        create.get_bone_group(target, "unmapped", theme='THEME13')
        create.get_bone_group(target, "active", theme='THEME09')

        source_group = source.pose.bone_groups.get('active')
        if not source_group:
            util.set_active_object(bpy.context, source)

            for target_group in target.pose.bone_groups:
                group_name = target_group.name
                group_color_set = target_group.color_set
                source_group = create.get_bone_group(source, group_name, theme=group_color_set)

        util.set_active_object(bpy.context, oactive)

    def set_select(rig, selection, state):
        for bone_name in selection:
            bone = rig.data.bones.get(bone_name)
            bone.select = state

    if source\
        and source.mode == 'POSE'\
        and target\
        and target.mode == 'POSE':
        try:
            block_retarget_bone_group_processing = True
            util.ensure_mode_is('POSE', object=target)
            util.ensure_mode_is('POSE', object=source)

            prepare_source_bone_groups(source, target)

            new_selected_target_bones = [b.name for b in target.data.bones if b.select]
            new_selected_source_bones = [b.name for b in source.data.bones if b.select]
            ...
            if not new_selected_source_bones:
                for bone in new_selected_target_bones:
                    link = prop.get(bone)
                    if link:
                        new_selected_source_bones.append(link)
            if not new_selected_target_bones:
                for bone in data.get_mtui_bones(target):
                    link = prop.get(bone)
                    if link in new_selected_source_bones:
                        new_selected_target_bones.append(bone)

            if prop.interactive_auto:
                if len(new_selected_target_bones) == 1 and len(new_selected_source_bones) == 1:
                    prop[new_selected_target_bones[0]] = new_selected_source_bones[0]

            selected_target_bones = new_selected_target_bones
            selected_source_bones = new_selected_source_bones
            adjust_retarget_mapping(prop)
        finally:
            block_retarget_bone_group_processing=False
    return
...
class ButtonSetSourceBone(bpy.types.Operator):
    bl_idname = "tamagoyaki.set_source_bone"
    bl_label ="Map source bone"
    bl_description =\
'''Map a selected pose bone from the source rig to >>%s<<

example: 

   - select the source bone for %s on the source rig
   - lookup %s in this list (you are already there!)
   - click this button'''
    ...
    @classmethod
    def description(cls, context, properties):
        bone = properties.target_bone
        return ButtonSetSourceBone.bl_description % (bone, bone, bone)
...
class ButtonClearBoneMap(bpy.types.Operator):
    bl_idname = "tamagoyaki.clear_bone_map"
    bl_label ="Clear"
    ...
    def execute(self, context):
        try:
            ...
            create.adjust_retarget_bone_groups(context, prop, clean=True)
...
class ButtonMappingDisplayDetails(bpy.types.Operator):
    bl_idname = "tamagoyaki.mapping_display_details"
    bl_label = ""
    bl_description =\
...
red when no mapping is in the table'''
...
class ButtonMappingDisplayMapper(bpy.types.Operator):
    bl_idname = "tamagoyaki.mapping_display_mapper"
    bl_label = ""
    bl_description =\
'''Enable the visual bone mapper

Detail: Show which bones are selected
    Also: Show which bones are affected.'''

    def set_bone_select(self, rig, selection, status):
    ...
    def reset_bone_mapper(self, context, source_bones, target_bones):
        scn = context.scene
        prop = scn.MocapProp
        source = bpy.data.objects[prop.source]
        target = bpy.data.objects[prop.target]

        self.set_bone_select(source, source_bones, False)
        self.set_bone_select(target, target_bones, False)

        check_active_bone_changed(context.scene, None, forced_check=True)
    ...
    def execute(self, context):
        ...
        source_bones = selected_source_bones.copy()
        target_bones = selected_target_bones.copy()

        if context.scene.MocapProp.show_bone_mapper:
            self.reset_bone_mapper(context, source_bones, target_bones)

        context.scene.MocapProp.show_bone_mapper = not context.scene.MocapProp.show_bone_mapper
        self.set_bone_select(source, source_bones, True)
        self.set_bone_select(target, target_bones, True)
...
class ButtonMappingEnable(bpy.types.Operator):
    bl_idname = "tamagoyaki.enable_retarget"
    bl_label = "Enable Visual Mapping"
    bl_description = \
'''When enabled: Select 2 bones (one in each rig)
then accept mapping in the panel

Note: leave visual retargetting by setting one of the armatures to Object mode'''
    ...
    def execute(self, context):
        ...
        if source and target:
            ...
            util.set_object_mode('POSE', object=source)
            util.set_active_object(context, target)
            util.set_object_mode('POSE', object=target)
            if active:
                util.set_active_object(context, target)
        return{'FINISHED'}
...
class ButtonGuessMapping(bpy.types.Operator):
    bl_idname = "tamagoyaki.guess_bone_map"
    bl_label =  "Automatic Mapping (guess)"
    bl_description =\
'''Guess the bone mapping from the source names.

Important: If no bones are mapped then 
Transfer Motion (see below) is disabled!'''
    bl_options = {'REGISTER', 'UNDO'}
...
class ButtonCopyOtherSide(bpy.types.Operator):
    bl_idname = "tamagoyaki.copy_other_side"
...
class AddAvatar(bpy.types.Operator):
    bl_idname = "tamagoyaki.add_avatar"
    bl_label = "Tamagoyaki"
    bl_description ="Create new Tamagoyaki Character"
    bl_options = {'REGISTER', 'UNDO'}

    quads : BoolProperty(
                name="with Quads",
                description="create Tamagoyaki with Quads",
                default=False,
    ...
    def get_preset(self, context, file):
        ...
        b_skeletonType  = sceneProps.tamagoyakiSkeletonType

        bpy.ops.script.python_file_run(filepath=file)

        self.quads      = sceneProps.tamagoyakiMeshType == 'QUADS'
    ...
    def execute(self, context):
        ...
        try:
            ...
            if initial_mode == 'POSE':
                ...
                bpy.ops.tamagoyaki.bone_preset_edit()
            ...
            util.select_single_object(arm_obj)
...
class TAMAGOYAKI_MT_AddMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        for file in glob.glob("%s/*.py" % RIG_PRESET_DIR):
            ...
            props      = layout.operator("tamagoyaki.add_avatar", text=label, icon=ICON_OUTLINER_OB_ARMATURE)
            props.file = file
...
class TAMAGOYAKI_MT_AddMenu2(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        props = layout.operator("tamagoyaki.add_avatar", text="Only Rig", icon=ICON_OUTLINER_OB_ARMATURE)
        props.quads   = False
        props.no_mesh = True
        props.rigtype = util.getAddonPreferences().target_system
...
class TAMAGOYAKI_MT_DevkitMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator("tamagoyaki.import_collada_devkit", text="Belleza",  icon=ICON_OUTLINER_OB_ARMATURE).devkit_type='BELLEZA'
...
def menu_import_tamagoyaki_shape(self, context):

    self.layout.operator(mesh.ButtonImportTamagoyakiShape.bl_idname)
    ...
    if user_templates == None:
        ...
        if user_templates == 'local':
            if True: #TODO: uncomment this -> get_blender_revision() < 278400:
                print("Use Tamagoyaki's native template system")
...
def register_templates():
    ...
    if 'toolset_pro' in dir(bpy.ops.sparkles):
        try:
            ...
            sparkles.register_template_path(path, 'Tamagoyaki Templates')
            log.warning("Added Tamagoyaki Templates to Sparkles Template list.")
            return 'sparkles'
        except:
            pass
    return 'local'
...
def sl_skeleton_func_export(self, context):
    self.layout.operator("tamagoyaki.export_sl_avatar_skeleton")

def sl_skeleton_func_import(self, context):
    self.layout.operator("tamagoyaki.import_sl_avatar_skeleton")
...
def import_submodule(module_name, package_name='tamagoyaki'):
...
classes = (
    ...
    DisplayTamagoyakiVersionOperator,
    DisplayTamagoyakiVersionMismatchOperator,
    DisplayTamagoyakiRigVersionOperator,
    WeightAcceptedHint,
    WeightIgnoredHint,
    ShapeTypeMorphHint,
...
def register():
    ...
    bpy.types.TOPBAR_MT_file_import.append(menu_import_tamagoyaki_shape)
    bpy.types.TOPBAR_MT_file_import.append(menu_import_tamagoyaki_devkits)
    ...
    if bpy.app.version_cycle != 'release':
        ...
        log.warning("Tamagoyaki:  Your Blender instance is in state '%s'" % bpy.app.version_cycle)
        log.warning("          We recommend to install this addon only on official")
        log.warning("          Blender releases from Blender.org")
        has_warnings = True

    bpy.utils.register_manual_map(tamagoyaki_docs)
    ...
    tamagoyaki_init    = __file__
    tamagoyaki_home    = os.path.dirname(tamagoyaki_init)
    tamagoyaki_apptemplates = os.path.join(tamagoyaki_home, "apptemplates")
    blender_scripts  = bpy.utils.user_resource('SCRIPTS', path="presets")

    tamagoyaki_themes = os.path.join(tamagoyaki_home, "interface_theme")
    destdir        = os.path.join(blender_scripts, "interface_theme")
    util.copydir(tamagoyaki_themes, destdir, overwrite=True)

    tamagoyaki_presets = os.path.join(tamagoyaki_home, "presets")
    destdir         = os.path.join(blender_scripts, __name__)
    util.copydir(tamagoyaki_presets, destdir, overwrite=True)


    rigs = os.path.join(destdir,"rigs")
    util.remove_file(os.path.join(rigs, "Basic.py"))
    util.remove_file(os.path.join(rigs, "Complete.py"))
    util.remove_file(os.path.join(rigs, "Skeleton.py"))

    if True: #TODO: uncomment this-> get_blender_revision() < 279000:

        util.copyblend(tamagoyaki_apptemplates, destdir, overwrite=True)
    else:

        path_app_templates = bpy.utils.user_resource(
            'SCRIPTS', path=os.path.join("startup", "bl_app_templates_user"),
            create=True,
        )
...
def register_handlers():
    ...
    bpy.app.handlers.load_post.append(fix_tamagoyaki_data_on_load)
    bpy.app.handlers.depsgraph_update_post.append(shape.check_dirty_mesh_on_load)
    bpy.app.handlers.depsgraph_update_post.append(check_active_bone_changed)
    bpy.app.handlers.frame_change_post.append(shape.update_on_framechange)
    print("Tamagoyaki Handlers registered")

def unregister_handlers():

    bpy.app.handlers.frame_change_post.remove(shape.update_on_framechange)
    bpy.app.handlers.depsgraph_update_post.remove(check_active_bone_changed)
    bpy.app.handlers.depsgraph_update_post.remove(shape.check_dirty_mesh_on_load)
    bpy.app.handlers.load_post.remove(fix_tamagoyaki_data_on_load)
...
def unregister():
    ...
    try:
        ...
        bpy.types.TOPBAR_MT_file_import.remove(menu_import_tamagoyaki_devkits)
    ...
    bpy.utils.unregister_manual_map(tamagoyaki_docs)
