...
log = logging.getLogger('tamagoyaki.rig')
connectlog = logging.getLogger('tamagoyaki.rig.connect')
registerlog = logging.getLogger("tamagoyaki.register")

log_cache = logging.getLogger('tamagoyaki.cache')
...
def adjustAvatarCenter(armobj):
    ...
    if not mPelvis:
        log.error("adjustSupportRig: mPelvis bone missing in armature %s. Maybe not an Tamagoyaki Rig?" % (armobj.name))
        return
    ...
    try:
        ...
        if layer == B_LAYER_IK_LEGS:
            ...
            if active.IKSwitchesProp.Enable_Hands in ['FK', 'GRAB']:
                props = row.operator("tamagoyaki.ik_apply", text="", icon=ICON_SNAP_ON)
                props.limb='HAND'
                props.symmetry='BOTH'
            return
        else:
...
def get_posed_bones(armobj):
    ...
    for b in armobj.pose.bones:
        if b.matrix_basis.decompose()[1].angle > 0.001:
            log.info("Found posed bone %s" % b.name)
...
class TamagoyakiFaceWeightGenerator(bpy.types.Operator):
    ...
    def execute(self, context):
        return TamagoyakiFaceWeightGenerator.generate(context, self.use_mirror_x, self.focus, self.gain, self.clean, self.limit, self.suppress_implode, 'ALL')
    ...
    def generate(context, use_mirror_x, focus, gain, clean, limit, suppress_implode, bone_selection_type):
        ...
        use_full_armature = not 'tamagoyaki' in arm
        ...
        selects, deforms, offsets = TamagoyakiFaceWeightGenerator.explode(arm, use_mirror_x, full_armature=use_full_armature, bone_names=selected_bone_names, focus=focus)
        ...
        TamagoyakiFaceWeightGenerator.store_parameters(arm, focus, gain, clean)

        util.ensure_mode_is("EDIT")
        if not suppress_implode:
            TamagoyakiFaceWeightGenerator.implode(arm, selects, deforms, offsets)
...
def convert_weight_groups(armobj, obj, armature_type=SLMAP):
    for group in obj.vertex_groups:
        tgt_name = map_sl_to_Tamagoyaki(group.name, type=armature_type, all=True)
...
class TamagoyakiFromManuelLab(bpy.types.Operator):

    bl_idname = "tamagoyaki.convert_from_manuel_lab"
    bl_label = "Convert to Tamagoyaki"
    bl_description = "Store current Slider settings in last selected Preset"

    def convert_weights(self, context, children):
        log.info("Converting %d children from Manuel_Lab to Tamagoyaki..." % len(children) )
        for ob in children:
    ...
    def execute(self, context):
        ...
        tamagoyakis = [arm for arm in util.get_meshes(context, type='ARMATURE', select=True, visible=True, hidden=False) if arm.get('tamagoyaki',None)]
        if len(tamagoyakis) == 0:
            tamagoyakis = [arm for arm in util.get_meshes(context, type='ARMATURE', select=None, visible=True, hidden=False) if arm.get('tamagoyaki',None)]
...
def tamagoyaki_split_manuel(context, active, island_min_vcount):
    omode  = active.mode
...
class TamagoyakiMergeWeights(bpy.types.Operator):

    bl_idname = "tamagoyaki.merge_weights"
...
class FocusOnBone(bpy.types.Operator):
    ...
    bl_idname = "tamagoyaki.focus_on_selected"
    bl_label = "Focus Bone"
    bl_options = {'REGISTER', 'UNDO'}
...
def armatureFromDictionary(context, armobj, dict, mocap=None, with_mbones=True, corr = None):
    ...
    if apply_as_restpose:
        ...
        bpy.ops.tamagoyaki.apply_as_restpose()
    elif adjust_tails:
...
def add_armature_preset(context, filepath):
    ...
    "import bpy\n"
    "import tamagoyaki\n"
    "from tamagoyaki import shape, util, rig\n"
...
class TamagoyakiAddPresetArmature(AddPresetBase, Operator):
...
class TamagoyakiUpdatePresetArmature(AddPresetBase, Operator):
...
class TamagoyakiRemovePresetArmature(AddPresetBase, Operator):
...
def fix_linebones_on_update(scene):
    ...
    if armobj and armobj.type=='ARMATURE' and "tamagoyaki" in armobj:
        if armobj.mode == "EDIT":
...
def fix_tamagoyaki_armature(context, armobj):
    log.debug("| fixing Tamagoyaki Armature %s" % armobj.name)
    scene=context.scene
    ...
    rig_version = armobj.get('tamagoyaki', 0)
    if rig_version < 4:
...
class ResetSpineBones(bpy.types.Operator):
    bl_idname = "tamagoyaki.reset_spine_bones"
...
class TamagoyakiCheckHierarchy(Operator):
    bl_idname = "tamagoyaki.check_hierarchy"
...
classes = (
    TamagoyakiFaceWeightGenerator,
    TamagoyakiFromManuelLab,
