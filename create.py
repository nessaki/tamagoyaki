...
log = logging.getLogger("tamagoyaki.create")
    ...
    if with_meshes:
        if quads == True:
            for obj in parts.values():
                try:
                    bpy.ops.tamagoyaki.quadify_avatar_mesh(mesh_id=obj["mesh_id"], object_name=obj.name)
                except:
                    print("Can not invoke Tamagoyaki quads conversion tool")
                    pass
...
def generate_system_meshes(context, arm_obj, armname, rigType, mesh_ids, progress):
    ...
    for mesh in MESHES.values():
        ...
        mat = propgroups.add_material_for(arm_obj.name, name, True, arm_obj.tamagoyakiMaterialProp.material_type, arm_obj.tamagoyakiMaterialProp.unique)
        if mat:
...
def create_face_controllers(arm, group):
    ...
    for symmetry in ["Left", "Right"]:
        ...
        if subtarget in pbones:
            ...
            set_source_range(con, 'SCALE',    [[0.5,1.5],    [0.5,1.5],      [0.5,1.50]] )
            set_mapping(con, 'X', 'X', 'Z')
            set_destination (con, 'LOCATION', [[0.015*xmirror,-0.015*xmirror],    [0.005,-0.005], [0.005,-0.005]] )
