...
class TamagoyakiDisplayInRestpose(bpy.types.Operator):
    ...
    bl_description = "Set Armature Pose to the default Tamagoyaki Pose for inspection\nNote: This operation neither changes the armature nor the object"
...
class TamagoyakiCleanupBinding(bpy.types.Operator):
    bl_idname      = "tamagoyaki.cleanup_binding"
...
class TamagoyakiAlterToRestpose(bpy.types.Operator):
...
def add_bind_preset(context, filepath):
    ...
    "import bpy\n"
    "import tamagoyaki\n"
    "from tamagoyaki import shape, util, bind\n"
...
class TamagoyakiAddPresetBind(AddPresetBase, Operator):
...
class TamagoyakiUpdatePresetBind(AddPresetBase, Operator):
...
class TamagoyakiRemovePresetBind(AddPresetBase, Operator):
...
class ArmatureJointPosStore(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_jointpos_store"
    bl_label = "Store Joint Edits"
    bl_description = tamagoyaki_apply_as_restpose_text
    ...
    def exec_imp(context, 
        ...
        try:
            ...
            if not delete_only: #not False
                ...
                if not only_meta:
                    shape.refreshTamagoyakiShape(context, refresh=True, target="")
...
class ArmatureJointPosRemove(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_jointpos_remove"
    ...
    def exec_imp(context, keep_edit_joints, affect_all_joints):
        ...
        try:
            ...
            arm_data_layers = [l for l in armobj.data.layers]
            if armobj != obj:
                omode = util.ensure_mode_is("OBJECT")
                util.set_active_object(context, armobj)
            amode = util.ensure_mode_is("OBJECT")
            ...
            armobj.data.layers = arm_data_layers
...
def configure_rig_display(props, context):
    ...
    if obj:
        ...
        if arm:
            ...
            objs = util.get_animated_meshes(context, arm, with_tamagoyaki=True, only_selected=True)
...
def fix_bone_layers(context, scene, lazy=True, force=False):
    ...
    try:
        ...
        if arm and 'tamagoyaki' in arm:
            ...
            objs = util.get_animated_meshes(context, arm, with_tamagoyaki=True, only_selected=active_object!=arm)
