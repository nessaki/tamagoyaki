...
log=logging.getLogger("tamagoyaki.copyrig")
...
def copy_tamagoyaki(self,
    ...
    log.info("+=========================================================================")
    log.info("| copy: Starting a %s Tamagoyaki copy from \"%s\"" % (tgt_rig_type, src_armature.name))
    log.info("+=========================================================================")

    no_mesh = not mesh_repair
    jointType = src_armature.RigProp.JointType
    skeletonType = src_armature.RigProp.SkeletonType
    ...
    util.set_active_object(context, src_armature)
    log.info("+=========================================================================")
    self.copy_skeletons(context, src_armature, self.targets, src_rig_type, tgt_rig_type, skeletonType, transfer_joints=True, sync=False)
    log.info("+=========================================================================")
...
def convert_to_tamagoyaki(self,
        context,
        tgt_rig_type,
        active_obj,
        src_armature,
    ...
    log.info("+=========================================================================")
    log.info("| convert: Starting a %s Tamagoyaki conversion from \"%s\"" % (tgt_rig_type, src_armature.name))
    log.info("+=========================================================================")
    ...
    if inplace_transfer:
        if self.applyRotation:
            log.warning("convert_to_tamagoyaki: Apply Visual transforms, Rotation and Scale to %s" % src_armature.name)
            old_state = util.select_hierarchy(src_armature) # All of them!
            bpy.ops.object.visual_transform_apply() #To get around some oddities with the apply tool (possible blender bug?)
        ...
        bpy.ops.pose.select_all(action="SELECT")
        bpy.ops.pose.copy()
        log.warning("copyrig-convert-tamagoyaki: copy pose from %s to pose buffer" % (src_armature.name) )
        src_armature.data.pose_position = 'REST'
        util.ensure_mode_is('OBJECT')

        tgt_armature = create.createAvatar(context, quads=True, use_restpose=use_restpose, rigType=tgt_rig_type, jointType=jointType, skeletonType=skeletonType, no_mesh=no_mesh)
        propgroups.gender_update(tgt_armature, self.use_male_shape)
        self.targets.append(tgt_armature)

    self.copy_skeletons(context, src_armature, self.targets, src_rig_type, tgt_rig_type, skeletonType, transfer_joints, sync=False)
    if active_is_arm:
        active_obj = self.targets[0]
    ...
    for arm in self.targets:
        ...
        self.move_objects_to_target(context, self.sources, arm, self.srcRigType) # Convert to tamagoyaki
        ...
        for child in util.get_animated_meshes(context, arm, with_tamagoyaki=False):
            ...
            if not is_fitted:
                for key in child.vertex_groups.keys():
                    if key in data.get_volume_bones():
                        ...
                        util.set_active_object(context, arm)
                        bpy.ops.tamagoyaki.armature_deform_enable(set='VOL')
                        break

        if not inplace_transfer:
            util.set_active_object(context, arm)

            bpy.ops.tamagoyaki.apply_shape_sliders()
    ...
    if action:
        active_obj.animation_data.action = action
        log.warning("copyrig-convert-tamagoyaki: Assigned Action %s to %s" % (action.name, active_obj.name) )
...
def update_tamagoyaki(self,
    ...
    def apply_scale_and_rotation(context, src_armature, sources):
        ...
        if not apply_set:
            log.info("| update tamagoyaki: Apply Rot&Scale not necessary (All children of %s are clean)" % oactive)
            return

        log.info("| update tamagoyaki: Apply Visual transforms, Rotation and Scale to %s" % oactive.name)
        select_states = util.get_select_and_hide(context.scene.objects, False, False, False)
        util.set_select(apply_set, reset=False)

        log.info("| update tamagoyaki: Apply Rot&Scale to %d children of [%s]" % (len(apply_set), oactive.name))
        bpy.ops.object.visual_transform_apply() #To get around some oddities with the apply tool (possible blender bug?)
        try:
            bpy.ops.object.transform_apply(location=False, rotation=True, scale=True)
        except Exception as e:
            log.error("| update tamagoyaki: Can not apply Object transformations to linked objects (Ignoring)")
    ...
    if mesh_repair:
        self.sources = [s for s in self.sources if not 'tamagoyaki-mesh' in s]
    ...
    self.copy_skeletons(context, src_armature, [tgt_armature], src_rig_type, tgt_rig_type, skeletonType, transfer_joints, sync=False, bone_store=bone_store)
    Skeleton.get_toe_hover(tgt_armature, reset=True, bind=True)

    tgt_armature.scale = src_armature.scale.copy()
    tgt_armature.RigProp.rig_use_bind_pose = use_bind_pose
    if shape_data:
...
def convert_sl(self,
    ...
    def get_child_set(context, arm, objects_to_rebind):
        if objects_to_rebind:
            ...
            child_set = util.get_animated_meshes(context, arm, with_tamagoyaki=False)
        return child_set
    ...
    if inplace_transfer:
        ...
        if self.srcRigType == MANUELMAP:
            ...
            for obj in src_armature.children:
                ...
                splitcount = rig.tamagoyaki_split_manuel(context, obj, 100)
    ...
    self.copy_skeletons(context, src_armature, self.targets, src_rig_type, tgt_rig_type, skeletonType, transfer_joints, sync=True)
    ...
    for arm in self.targets:
        ...
        if not inplace_transfer:
            ...
            bpy.ops.tamagoyaki.alter_to_restpose()
...
class ImportColladaDevkit(bpy.types.Operator, ImportHelper):
    bl_idname = "tamagoyaki.import_collada_devkit"
...
class RigTransferConfigurator (util.ExpandablePannelSection):
    bl_idname = "tamagoyaki.rig_transfer_configurator"
    bl_label = "Rig Transfer"


class ButtonCopyTamagoyaki(bpy.types.Operator):
    '''
    Convert/Update/Cleanup Rigs
    '''
    bl_idname = "tamagoyaki.copy_rig"
    bl_label  = "Copy Rig"
    bl_description = messages.tamagoyaki_copy_rig_bl_description
    ...
    transferMeshes : BoolProperty(
        ...
        default=False,
        description=messages.tamagoyaki_copy_rig_transferMeshes
    )

    transferJoints : BoolProperty(
        name = "Transfer Joints",
        default=True,
        description = messages.tamagoyaki_copy_rig_transferJoints,
    )
    ...
    mesh_repair     : BoolProperty(
        name        = "Rebuild Tamagoyaki Meshes",
        description = messages.tamagoyaki_copy_rig_mesh_repair,
        default     = False
    )

    show_offsets      : BoolProperty(
        name="Show Offsets",
        description = messages.tamagoyaki_copy_rig_show_offsets,
        default     = False
    )

    sl_bone_ends : BoolProperty(
        name="Enforce SL Bone ends",
        description = messages.tamagoyaki_copy_rig_sl_bone_ends,
        default     = True
    )
    ...
    def draw_generic(op, context, layout, src_armature, targets, repair=False):
        ...
        scene = context.scene
        all_mesh = util.get_animated_meshes(context, src_armature, with_tamagoyaki=True, only_selected=False, return_names=False)
        custom_mesh = util.get_animated_meshes(context, src_armature, with_tamagoyaki=False, only_selected=False, return_names=False)
        ...
        if op or len(targets) > 0 or srcRigType!='AVASTAR':
            ...
            if srcRigType == 'AVASTAR':
                ...
                brow.prop(src_armature.RigProp, "SkeletonType", text="")

                bcol = ibox.column(align=True)
                if ButtonCopyTamagoyaki.sliders_allowed(context):
        ...
        if not op:
            ...
            if len(targets) == 0:
                if "tamagoyaki" in src_armature:
                    ...
                    if rig_version != None:
                        note = "%s %s(%s)" %("Tamagoyaki " if repair else "Outdated Rig", rig_version, rig_id, )
                        try:
                        ...
                        rig_version='%s %s' % ('Tamagoyaki', rig_version)
                    else:
                        note = None
                        rig_version='unknown'
                    ...
                    if origin and Vector(origin.head).magnitude > MIN_JOINT_OFFSET:
                        ...
                        op=row.operator("tamagoyaki.adjust_armature_origin", icon=ICON_FILE_REFRESH, text="")
                ...
                col.prop(updateRigProp, "applyRotation")
                label="Copy to Tamagoyaki Rig"

            col = box.column(align=True)
            row = col.row(align=True)

            props = row.operator("tamagoyaki.copy_rig", text=label)
            ...
            props.SkeletonType  = updateRigProp.SkeletonType
            ...
            if note:
                col = box.column(align=True)
                row = col.row(align=True)
                op=row.operator("tamagoyaki.generic_info_operator", text='', icon=ICON_INFO, emboss=False)
                op.msg=msg
                op.url="/help/armature-info/"
                op=row.operator("tamagoyaki.generic_info_operator", text=note, emboss=False)
                op.msg=msg
                op.url="/help/armature-info/"
    ...
    def move_objects_to_target(self, context, sources, tgt_armature, source_rig_type, src_armature = None):
        ...
        def get_custom_set(sources, container):
            if not sources:
                return []
            return [s for s in sources if not ('tamagoyaki-mesh' in s or (container and s.parent == container))]

        def get_system_set(sources, container):
            if not sources:
                return []
            return [s for s in sources if 'tamagoyaki-mesh' in s or (container and s.parent == container)]
        ...
        if len(system_set) > 0:
            ...
            log.warning("|  Move %d Tamagoyaki meshes to %s %s" % (len(system_set), tgt_armature.type, tgt_container.name) )
    ...
    def move_subset_to_target(self, context, sources, tgt_armature, source_rig_type, tgt_parent):
        with set_context(context, tgt_armature, 'OBJECT'):
            ...
            for obj in sources:
                ...
                if not have_armature_modifier and obj.type in ['MESH', 'ARMATURE']:
    ...
    def transfer_joint_info(self, context, src_armature, tgt_armature, src_rig_type, bone_store, sync):
        ...
        def get_tamagoyaki_bone_id(key, ebones):

            bid = key #if 'tamagoyaki' in src_armature else map_sl_to_Tamagoyaki(key, src_rig_type, all=False)
            if bid is None:
        ...
        def clear_connected(ebones, bone_store):

            def preserve_connect_info(connected, bone, bone_store):
                bone_data = bone_store.get(bone.name)
                if bone_data:
                    use_connect = bone_data[BONE_DATA_CONNECT]
                    if use_connect:
            ...
            for bone in ebones:
                if bone.use_connect:
                    preserve_connect_info(connected, bone, bone_store)
        ...
        def adjust_bone_group(tgt_armature, bone_data, ebone, pbone):
            ...
            if self.show_offsets:
                ...
                util.gp_draw_line(context, restpose_head, ebone.head, pname='tamagoyaki', lname='tamagoyaki', color_index = group_index)
                util.gp_draw_line(context, restpose_tail, ebone.tail, pname='tamagoyaki', lname='tamagoyaki', color_index = group_index)
        ...
        def update_from_bone_store(ebones, pbones, bone_store):
            ...
            connected = clear_connected(ebones, bone_store)
        ...
        log.info("| Adjust Tamagoyaki Bones missing from Source Rig [%s] " % (src_armature.name))
        madjust_counter = 0
        cadjust_counter = 0
        ...
        for key in bone_names:
            ...
            if True:
                ...
                if self.show_offsets:
                    ...
                    util.gp_draw_line(context, head, tgt_ebone.head, pname='tamagoyaki', lname='tamagoyaki', color_index = group_index)
    ...
    def copy_skeleton(self, context, src_armature, tgt_armature, src_rig_type, tgt_rig_type, skeletonType, bone_store, transfer_joints=True, sync=True):
        ...
        if self.inplace_transfer:
            ...
            if self.sl_bone_ends:
                log.info("Adjust imported bone ends to Match the Tamagoyaki default")
                ebones = tgt_armature.data.edit_bones
                self.untag_boneset(ebones)
            ...
            rig.fix_tamagoyaki_armature(context, tgt_armature)
            ...
            if self.transferJoints:
                ...
                delete_only = False#not ButtonCopyTamagoyaki.sliders_allowed(context)
            ...
            for bone_data in  bone_store.values():
                key = bone_data[BONE_DATA_NAME]
                bid =  map_sl_to_Tamagoyaki(key, src_rig_type, all=False)
        ...
        util.ensure_mode_is('OBJECT')
        if self.transferJoints and not self.inplace_transfer and not 'tamagoyaki' in src_armature:
    ...
    def init(self, context):
        ...
        if 'tamagoyaki' in self.src_armature and self.src_armature['tamagoyaki'] > 2:
        ...
        self.targets  = [arm for arm in self.selected if arm.type=='ARMATURE' and 'tamagoyaki' in arm and arm != self.src_armature]
    ...
    def invoke(self, context, event):
        def name_to_mesh(context, names):
            result = []
            for name in names:
        ...
        system_mesh_names, animated_mesh_names = util.get_animated_elements(context, armature, only_visible=False, return_names=True)
        if mesh_repair:

            animated_mesh_names = [m for m in animated_mesh_names if m not in system_mesh_names]
            system_mesh_names=[]
        ...
        mesh.unbind_from_armature(self, context, animated_meshes, keep_tamagoyaki_properties=True)
    ...
    def execute(self, context):
        ...
        def fix_tinker_animations(armobj):
            if armobj.animation_data.action and "Tinker" in armobj.data.bones:
                tinker_curves = [c for c in armobj.animation_data.action.fcurves if "PelvisInv" in c.data_path]
                for fcurve in tinker_curves:
                    fcurve.data_path = fcurve.data_path.replace("PelvisInv", "Tinker")
                    if fcurve.group.name == "PelvisInv":
                        fcurve.group.name = "Tinker"
        ...
        if self.srcRigType != 'AVASTAR':
            ...
            if self.inplace_transfer:
                ...
                tgt_armature = update_tamagoyaki(
                    self,
                    context,
                    tgt_rigtype,
                    self.active,
        ...
        if tgt_armature:
            ...
            fix_tinker_animations(tgt_armature)
...
def extract_bone_data(context, armature, bone_store, rot):
    ...
    def extract_data(armature, name, bone_info, rot):
        ...
        bone_info[BONE_DATA_MATRIX] = rot @ pbone.matrix.copy()
...
def create_transfer_preset(layout):
    ...
    if last_select not in ["Transfer Presets", "Presets"]:
        ...
        row.operator("tamagoyaki.transfer_presets_remove", text="", icon=ICON_REMOVE).remove_active = True
...
def add_transfer_preset(context, filepath):
    ...
    file_preset.write("armobj.RigProp.SkeletonType = '%s'\n" % armobj.RigProp.SkeletonType)
...
class TamagoyakiAddPresetTransfer(AddPresetBase, Operator):
...
class TamagoyakiUpdatePresetTransfer(AddPresetBase, Operator):
...
class TamagoyakiRemovePresetTransfer(AddPresetBase, Operator):
    ...
    for dir in paths:
        ...
        for preset in presets:
            ...
            if os.path.isfile(path) and path.endswith('.py') or path.endswith('.blend'):
                ...
                col.alert = alert
                prop = col.operator("tamagoyaki.execute_devkit_preset", text="%s" % (name), icon=icon)
                prop.filepath = path
...
def add_devkit_preset(context, filepath, allow_overwrite):
    ...
    file_preset.write("kitprop.SkeletonType='%s'\n"       % (kitprop.SkeletonType))
    file_preset.write("kitprop.tgtJointType='%s'\n"       % (kitprop.tgtJointType))
    file_preset.write("kitprop.tgtSkeletonType='%s'\n"    % (kitprop.tgtSkeletonType))
    ...
    newpath = os.path.join(dirname, 'hidden-%s'%basename)
...
class WM_OT_AVASTAR_RemoveDevkitPreset(Operator):
    ...
    def description(cls, context, properties):
        name = bpy.path.display_name(properties.get('filepath'))
        text = \
...
All actual developerkit files are preserved.''' % name
    ...
    def execute(self, context):
        if os.path.exists(self.filepath):
            basename = 'hidden-%s'%os.path.basename(self.filepath)
...
class LoadDevkitPreset(Operator):
    """Add a character from a development kit"""
    bl_idname = "tamagoyaki.load_devkit_preset"
    bl_label = "Add Devkit"
...
class TamagoyakiAddPresetDevkit(AddPresetBase, Operator):
...
class TamagoyakiUpdatePresetDevkit(AddPresetBase, Operator):
...
class TamagoyakiRemovePresetDevkit(AddPresetBase, Operator):
...
classes = (
    ...
    TamagoyakiUpdatePresetDevkit,
    TamagoyakiRemovePresetDevkit,
...
def register():
    ...
    bpy.types.WM_MT_button_context.append(menu_devkit_presets)

def unregister():
    from bpy.utils import unregister_class

    bpy.types.WM_MT_button_context.remove(menu_devkit_presets)
