...
log=logging.getLogger("tamagoyaki.updaterig")
registerlog = logging.getLogger("tamagoyaki.register")

class UpdateTamagoyakiRig(bpy.types.Operator):
    '''
    Convert/Update/Cleanup Rigs
    '''
    bl_idname = "tamagoyaki.update_rig"
    bl_label  = "Update Rig"
    bl_description = "Check Rig for compatibility issues with current Tamagoyaki version\n"\
                   + "Optional: Fix issues where necessary to make sure Tamagoyaki works as intended."
