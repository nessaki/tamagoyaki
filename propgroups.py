...
log = logging.getLogger('tamagoyaki.propgroups')
...
def update_rig_pose_type(self, context):
    ...
    obj = util.get_active_object(context)
    armobj = util.get_armature(obj)
    if armobj and 'tamagoyaki' in armobj:
...
def slider_options(self, context):
    ...
    if obtype=='ARMATURE':
        items=[
                ...
                ('SL',    "Avatar Shape", "Use Tamagoyaki Sliders to simulate the SL Avatar shape on all of Armature's Custom Meshes")
              ]
    else:
        items=[
                ('NONE',  "No Sliders", 'Disable Tamagoyaki Sliders from Selected Meshes'),
                ('SL',    "Avatar Shape", 'Use Tamagoyaki Sliders to simulate the SL Avatar shape on Selected Meshes')
              ]
    return items
...
def set_tamagoyaki_materials(context):
    obj = context.active_object
    ...
    util.ensure_tamagoyaki_repository_is_loaded()
    parts = util.findTamagoyakiMeshes(obj) if obj.type=='ARMATURE' else {obj.name: obj for obj in context.selected_objects}
    for name in parts:
        part = parts[name]
        ...
        if mat:
            ...
            part.tamagoyakiMaterialProp.material_type = material_type
            part.tamagoyakiMaterialProp.unique = unique
...
(*): tol is the maximum Cartesian distance error to tolerate"""
    ...
    with_reference_frame : BoolProperty(
                  ...
                  description = "Prepend a generated BVH reference frame before the first animation frame.\n\n"\
                              ...
                              + "Tamagoyaki generate a reference frame as needed.\n"\
...
class ScenePropGroup(bpy.types.PropertyGroup):
    ...
    tamagoyakiMeshType : EnumProperty(
        items=(
        ...
        name="Create Avatar",
        description="Create a new Tamagoyaki character",
        default='TRIS')

    tamagoyakiRigType : EnumProperty(
        items=RigTypeItems,
        name="Rig Type",
        description= "The set of used Bones",
        default='EXTENDED')

    tamagoyakiJointType : EnumProperty(
        items=(
    ...
    collada_export_boneroll : BoolProperty(default=False, name= "Export Bone Roll",
                            description="Export with Tamagoyaki Bone Roll values (Experimental)"
                           )
    collada_export_layers : BoolProperty(default=False, name= "Export Bone Layers",
                            description="Export with Tamagoyaki Bone Layers"
                           )
    ...
    panel_preset : EnumProperty(
                    ...
                    items=(
                            ...
                            ('RETARGET', 'Motion Transfer', 'Prepare the Panels for Motion Transfer (Retarget, transform Animations from foreign Rigs to Tamagoyaki)'),
    ...
    with_image_bake : BoolProperty(
                   ...
                   description = "Bake Materials into Textures\n"
                               ...
                               + "Hower, Tamagoyaki uses a cache here, hence follow up exports\n"
...
class UpdateRigPropGroup(bpy.types.PropertyGroup):
    ...
    adjust_origin : EnumProperty(
        ...
        name="Origin",
        description="Matches the Tamagoyaki Root Bone with the Tamagoyaki Origin location.\nThis must be done to keep the Sliders working correct",
    ...
    mesh_repair     : BoolProperty(
        ...
        description = "Reconstruct all missing Tamagoyaki Meshes.\nThis applies when Tamagoyaki meshes have been removed from the original rig\n\nCAUTION: If your character has modified joints then the regenerated Tamagoyaki meshes may become distorted!",
        default     = False
    )
...
that already exists in your setup, then allow Tamagoyaki
...
then allow Tamagoyaki to replace the content of the file
...
def type_changed(self, context):
    ...
    set_tamagoyaki_materials(context)
...
class ObjectPropGroup(bpy.types.PropertyGroup):
    ...
    apply_to_tamagoyaki_meshes : BoolProperty(
        default = True,
        name="Also Tamagoyaki Meshes",
        description='''enabled: Apply also to Tamagoyaki Meshes.\nDisabled: apply only to Custom Meshes'''
    )
...
class IKSwitchesPropGroup(bpy.types.PropertyGroup):
    ...
    def update_legs_ik(self, context):
        ...
        self.Enable_Legs = rig.ButtonEnableIK.set_ik_status(context, self.legs_ik_enabled, 'Ankle', self.Enable_Legs, val)
    ...
    def update_arms_ik(self, context):
        ...
        self.Enable_Arms = rig.ButtonEnableIK.set_ik_status(context, self.arms_ik_enabled, 'Wrist', self.Enable_Arms, val)
...
def unregister():
    ...
    if hasattr(bpy.types.Object,'tamagoyakiMaterialProp'): del bpy.types.Object.tamagoyakiMaterialProp
