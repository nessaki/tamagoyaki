...
log = logging.getLogger('tamagoyaki.weights')
...
def update_with_hidden_tamagoyaki_meshes(self, context):
...
g_with_listed_tamagoyaki_meshes = BoolProperty(
...
g_with_hair = BoolProperty(
            name="Hair",
            default=False,
            description="Include Tamagoyaki hair mesh as Weight Source" )
...
g_with_eyes = BoolProperty(
            name="Eyes",
            default=False,
            description="Include Tamagoyaki eye meshes as Weight Source" )

g_with_eyelashes = BoolProperty(
            name="Eyelashes",
            default=False,
            description="Include Tamagoyaki eyelash meshes as Weight Source" )

g_with_upper_body = BoolProperty(
            name="Upper Body",
            default=True,
            description="Include Tamagoyaki upper body mesh as Weight Source" )

g_with_lower_body = BoolProperty(
            name="Lower Body",
            default=True,
            description="Include Tamagoyaki lower body mesh as Weight Source" )

g_with_skirt = BoolProperty(
            name="Skirt",
            default=False,
            description="Include Tamagoyaki skirt mesh as Weight Source" )
...
def mirrorBoneWeightsFromOppositeSide(context, operator, use_topology=False):
    obj        = context.object
...
def smart_mirror_vgroup(context, armobj, target, targetBoneNames, allVerts=True, clearTargetWeights=True, submeshInterpolation=True):
    ...
    clean_target_groups = []
    sourceBoneNames     = [util.get_mirror_name(gn) for gn in targetBoneNames]

    print("Target bones:", targetBoneNames)
    print("Source bones:", sourceBoneNames)


    if target.active_shape_key_index != 0:
        osk = target.active_shape_key_index
        target.active_shape_key_index = 0
        util.set_object_mode("EDIT", object=target)
        util.set_object_mode("OBJECT", object=target)
        target.active_shape_key_index = osk
    ...
    for vertex in target.data.vertices:
        if allVerts or vertex.select:
            ...
            copied=[]
            for sn,w in vmin[1].items():
                if sn in sourceBoneNames:
                    gn = util.get_mirror_name(sn)
                    copied.append(gn)
            ...
            if clearTargetWeights and not allVerts:
                for gn in [gn for gn in sourceBoneNames if gn in target.vertex_groups and gn not in copied]:
...
def get_weights(source, target, point, submesh = False, restrictTo=None, depsgraph=None):
    ...
    if submesh:
        ...
        vw = interpolated_weights(source, polygon, loc)
        ...
        for vidx, interpw in vw.items():
            ...
            for grp in v.groups:
                gname = target.vertex_groups[grp.group].name
        ...
        vtx = min(polygon.vertices, key=lambda v: (p - vertices[v].co).length)
...
def interpolated_weights(obj, polygon, loc):
...
class ButtonDeletePhysics(bpy.types.Operator):
    bl_idname = "tamagoyaki.delete_physics"
...
class ButtonRebaseShapekey(bpy.types.Operator):
    bl_idname = "tamagoyaki.rebase_shapekey"
...
class ButtonRegeneratePhysics(bpy.types.Operator):
    bl_idname = "tamagoyaki.regenerate_physics"
...
class ButtonEnablePhysics(bpy.types.Operator):
    bl_idname = "tamagoyaki.enable_physics"
...
class ButtonGeneratePhysics(bpy.types.Operator):
    bl_idname = "tamagoyaki.generate_physics"
...
def add_fitting_preset(context, filepath):
    ...
    file_preset.write(
'''#Generated by Tamagoyaki %s with Blender %s
...
class TamagoyakiAddPresetFitting(AddPresetBase, Operator):
...
class TamagoyakiUpdatePresetFitting(AddPresetBase, Operator):
...
class TamagoyakiRemovePresetFitting(AddPresetBase, Operator):
...
def copyBoneWeightsToSelectedBones(target, sources, selectedBoneNames, submeshInterpolation=True, allVerts=True, clearTargetWeights=True):
    ...
    if len(clones) == 0:
        raise "Please ensure that at least one part of the Tamagoyaki mesh is visible.\n Then try again."
...
class ButtonCopyBoneWeights(bpy.types.Operator):
    bl_idname = "tamagoyaki.copy_bone_weights"
    bl_label = "Copy Weights"
    bl_description = "Various Copy tools operating on Bone weight groups (use redo panel for setttings)"
    bl_options = {'REGISTER', 'UNDO'}
        ...
        try:
            if self.weightCopyType == "BONES":
                ...
                if self.weightCopyType == "CLEAR":
                    c = removeBoneWeightsFromSelectedBones(context, self, not onlySelectedVerts, boneNames)
...
class ButtonClearBoneWeightGroups(bpy.types.Operator):
    bl_idname = "tamagoyaki.clear_bone_weight_groups"
...
class ButtonClearBoneWeights(bpy.types.Operator):
    bl_idname = "tamagoyaki.clear_bone_weights"
...
class ButtonSwapWeights(bpy.types.Operator):
    bl_idname = "tamagoyaki.swap_bone_weights"
...
class ButtonMirrorBoneWeights(bpy.types.Operator):
    bl_idname = "tamagoyaki.mirror_bone_weights"
    ...
    def execute(self, context):
        ...
        try:
            c = mirrorBoneWeightsFromOppositeSide(context, self, context.object.data.use_mirror_topology)
...
class ButtonCopyWeightsFromRigged(bpy.types.Operator):
    bl_idname = "tamagoyaki.copy_weights_from_rigged"
...
class ButtonCopyWeightsFromSelected(bpy.types.Operator):
    bl_idname = "tamagoyaki.copy_weights_from_selected"
...
class ButtonWeldWeightsFromRigged(bpy.types.Operator):
    ...
    bl_description = messages.tamagoyaki_weld_weights_from_rigged_description
    bl_options = {'REGISTER', 'UNDO'}
...
class ButtonEnsureMirrorGroups(bpy.types.Operator):
    bl_idname = "tamagoyaki.ensure_mirrored_groups"
...
class ButtonRemoveGroups(bpy.types.Operator):
    bl_idname = "tamagoyaki.remove_empty_groups"
...
class ButtonGenerateWeights(bpy.types.Operator):
    ...
    def draw_fitting_section(context, layout):
        ...
        if not "tamagoyaki" in armobj:
            col.label(text="Object Not rigged to Tamagoyaki", icon=ICON_INFO)
            return

        if "tamagoyaki-mesh" in obj:
            col.operator("tamagoyaki.convert_to_custom", icon=ICON_FREEZE)
            return
        ...
        if  obj.mode not in ['WEIGHT_PAINT', 'EDIT']:
            ...
            col.alert=False
            col.enabled=True
            col.operator("tamagoyaki.bone_preset_fit", text="Adjust Viewport", icon = ICON_MODIFIER )
            return
        ...
        row = col.row(align=True)
        row.operator("tamagoyaki.generate_physics")
        if not physics:
            row = col.row(align=True)
            row.operator("tamagoyaki.enable_physics")
        else:
            row.operator("tamagoyaki.delete_physics", text="", icon=ICON_X)
            ...
            p=row.operator("tamagoyaki.fitting_bone_selected_hint", text="", icon=icon)
            p.bone ='BUTT'
            p.bone2=''
        ...
        if (context.scene.SceneProp.panel_preset!='FIT'):
            ...
            row.operator("tamagoyaki.bone_preset_fit", text="Adjust Viewport", icon = ICON_MODIFIER )
        else:
            ...
            for set in bone_sets:
                ...
                for bname in set:
                    ...
                    if display:
                        ...
                        p      = row.operator("tamagoyaki.fitting_bone_delete_pgroup", text="", icon=icon)
                        p.bname = bname


        col = layout.column(align=True)
        row = col.row(align=True)
        row.operator("tamagoyaki.distribute_weights", icon=ICON_SHAPEKEY_DATA)
        row.operator("tamagoyaki.smooth_weights", icon=ICON_MOD_SMOOTH)
    ...
    def display_weights_from_meshes(context, armobj, prop, layout, weight_source_type, is_redo=False):
        ...
        if len(available_meshes) > 0:
            ...
            can_run = selected_mesh_count > 0 or with_listed_tamagoyaki_meshes
        else:
            col.alert=True
    ...
    def assign_tamagoyaki_meshes(self, context):
        props = context.scene.MeshProp
    ...
    def execute(self, context):
        ...
        for target in [ ob for ob in context.selected_objects if ob.type=='MESH']:
            ...
            if props.weightSourceSelection != 'FACEGEN':
                ...
                done = rig.TamagoyakiFaceWeightGenerator.generate(context, self.use_mirror_x, self.focus, self.gain, self.clean, self.limit, self.suppress_implode, self.weightBoneSelection)
                pbone = armobj.data.bones.active
                if pbone:
