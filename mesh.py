...
from . import animation, armature_util, bind, const, create, data, messages
from . import util, rig, shape, bl_info, weights, propgroups
...
log_export = logging.getLogger('tamagoyaki.export')
log = logging.getLogger('tamagoyaki.mesh')
...
g_purge_data_on_unbind = BoolProperty(
            default     = False,
            name        = "Cleanup",
            description = '''Remove Tamagoyaki custom properties.
...
def create_collada_layout(context, layout, attached, targets, on_toolshelf=False):
    ...
    if ui_level == UI_SIMPLE:
        ...
        row.operator("tamagoyaki.expert_preset", text='', icon=ICON_LIGHT_DATA)
        row.operator("tamagoyaki.expert_preset", text="Enable Advanced mode")
    else:
    ...
    if armobj and armobj.get('has_bind_data'):
        ...
        if not is_in_restpose and not use_bind_pose:
            ...
            icol=ibox.column()
            icol.alert=True
            icol.operator("tamagoyaki.reset_to_restpose", text='Set Neutral Shape')
...
def displayShowBones(context, layout, active, armobj, with_bone_gui=False, collapsible=True):
    ...
    if ui_level > UI_SIMPLE:
        ...
        irow.alignment = 'LEFT'
        irow.prop(armobj,"show_in_front", text="In front", toggle=True)
        irow.prop(armobj.data,"show_bone_custom_shapes", text="Shape", toggle=True)

        show_icon = ICON_HIDE_OFF if context.space_data.overlay.show_relationship_lines else ICON_HIDE_ON
        enable_icon = ICON_RADIOBUT_ON if context.scene.MeshProp.enableBoneConstraints else ICON_RADIOBUT_OFF
        irow = split.row(align=True)
...
class ButtonRigdisplaySpecialBoneGroups(bpy.types.Operator):
    bl_idname = "tamagoyaki.rig_display_special_bone_groups"
    bl_label = ""
...
class ButtonRigdisplayDeformBoneGroups(bpy.types.Operator):
    bl_idname = "tamagoyaki.rig_display_deform_bone_groups"
    bl_label = ""
    bl_description = "Hide/Unhide Special Bone Groups"
...
class ButtonRigdisplayAnimationBoneGroups(bpy.types.Operator):
    bl_idname = "tamagoyaki.rig_display_animation_bone_groups"
    bl_label = ""
...
class ButtonRigdisplayIKBoneGroups(bpy.types.Operator):
    bl_idname = "tamagoyaki.rig_display_ik_bone_groups"
    bl_label = ""
    bl_description = "Hide/Unhide IK Bone Groups"
    ...
    def execute(self, context):
        ButtonRigdisplayIKBoneGroups.visible = not ButtonRigdisplayIKBoneGroups.visible
        return{'FINISHED'}


def displayBoneDetails(context, box, armobj, ui_level, for_retarget=False):


    def do_display_ik_bone_groups(context, armobj, ui_level, layout):
        col = layout.column(align=True)
        props     = util.getAddonPreferences()
        sceneProps = context.scene.SceneProp
        if armobj.RigProp.RigType != 'BASIC':
            rig.create_ik_button(col.row(align=True), armobj, B_LAYER_IK_HAND)
        rig.create_ik_button(col.row(align=True), armobj, B_LAYER_IK_ARMS)
        rig.create_ik_button(col.row(align=True), armobj, B_LAYER_IK_LEGS)
        if armobj.RigProp.RigType != 'BASIC':
            rig.create_ik_button(col.row(align=True), armobj, B_LAYER_IK_LIMBS)
        rig.create_ik_button(col.row(align=True), armobj, B_LAYER_IK_FACE)
        col.separator()
        col.prop(armobj.IKSwitchesProp,'snap_on_switch')
        col.separator()

    def do_display_animation_bone_groups(context, armobj, ui_level, layout, for_retarget=False):
        col = layout
        props     = util.getAddonPreferences()
        sceneProps = context.scene.SceneProp
    ...
    txt = "Retarget Bone Groups" if for_retarget else "Animation Bone Groups"
    row.operator("tamagoyaki.rig_display_animation_bone_groups", text=txt)
    ...
    if armobj.RigProp.RigType != 'REFERENCE':
        ...
        col = box.column(align=True)
        row=col.row(align=True)
        row.operator("tamagoyaki.rig_display_ik_bone_groups", icon = util.get_collapse_icon(display_ik_bone_groups))
        row.operator("tamagoyaki.rig_display_ik_bone_groups", text="IK Bone Groups")

        if display_ik_bone_groups:
            do_display_ik_bone_groups(context, armobj, ui_level, col)
    ...
    col = box.column(align=True)
    row=col.row(align=True)
    row.operator("tamagoyaki.rig_display_deform_bone_groups", icon = util.get_collapse_icon(display_deform_bone_groups))
    row.operator("tamagoyaki.rig_display_deform_bone_groups", text="Deform Bone Groups")
    ...
    if ui_level > UI_SIMPLE:
        ...
        col = box.column(align=True)
        row=col.row(align=True)
        row.operator("tamagoyaki.rig_display_special_bone_groups", icon = util.get_collapse_icon(display_special_bone_groups))
        row.operator("tamagoyaki.rig_display_special_bone_groups", text="Special Bone Groups")
...
class PanelAvatarShapeIO(bpy.types.Panel):
    ...
    def draw_generic(op, context, arm, layout):
        ...
        col = box.column(align=True)
        col.alignment='LEFT'
        col.operator("tamagoyaki.load_props", icon=ICON_IMPORT)
        split = col.split(factor=0.6, align=True)
        split.operator("tamagoyaki.save_props",text="Export Shape to", icon=ICON_EXPORT).destination=meshProp.save_shape_selection
        split.prop(meshProp, "save_shape_selection", text="", toggle=False)
        col.operator("tamagoyaki.refresh_character_shape", icon=ICON_FILE_REFRESH)
...
class ButtonBoneDisplayDetails(bpy.types.Operator):
    bl_idname = "tamagoyaki.bone_display_details"
    bl_label = ""
...
class ButtonColladaDisplayTextures(bpy.types.Operator):
    bl_idname = "tamagoyaki.collada_display_textures"
    bl_label = ""
...
class ButtonColladaDisplayAdvanced(bpy.types.Operator):
    bl_idname = "tamagoyaki.collada_display_advanced"
    bl_label = ""
    bl_description = \
...
class ButtonColladaResetAdvanced(bpy.types.Operator):
    bl_idname = "tamagoyaki.collada_reset_advanced"
    bl_label = ""
    bl_description = "reset panel values to Tamagoyaki defaults"
...
class ButtonColladaDisplayUnsupported(bpy.types.Operator):
    bl_idname = "tamagoyaki.collada_display_unsupported"
    bl_label = ""
...
class ButtonSmoothWeights(bpy.types.Operator):
    bl_idname = "tamagoyaki.smooth_weights"
...
class ButtonDistributeWeights(bpy.types.Operator):
    bl_idname = "tamagoyaki.distribute_weights"
...
class ButtonBasicPreset(bpy.types.Operator):
    bl_idname = "tamagoyaki.basic_preset"
...
class ButtonAdvancedPreset(bpy.types.Operator):
    bl_idname = "tamagoyaki.expert_preset"
...
class ButtonAllPreset(bpy.types.Operator):
    bl_idname = "tamagoyaki.all_preset"
    bl_label = "Experimental"
    bl_description = '''Show all Tamagoyaki features.
...
class ButtonBonePresetSkin(bpy.types.Operator):
    bl_idname = "tamagoyaki.bone_preset_skin"
...
guse_default_locks    = BoolProperty(name="Default Locks", description="Set Tamagoyaki default constraints and Bone connects", default=False)

class ButtonBonePresetAnimate(bpy.types.Operator):
    bl_idname = "tamagoyaki.bone_preset_animate"
...
class ButtonBonePresetScrub(bpy.types.Operator):
    bl_idname = "tamagoyaki.bone_preset_scrub"
...
class ButtonBonePresetRetarget(bpy.types.Operator):
    bl_idname = "tamagoyaki.bone_preset_retarget"
...
class ButtonBonePresetEdit(bpy.types.Operator):
    bl_idname = "tamagoyaki.bone_preset_edit"
    bl_label = "Joint Edit"
...
class ButtonBonePresetFit(bpy.types.Operator):
    bl_idname = "tamagoyaki.bone_preset_fit"
    ...
    def execute(self, context):
        ...
        util.set_armature_layers(armobj, [B_LAYER_DEFORM])
...
class ButtonFindUnweighted_old(bpy.types.Operator):
    bl_idname = "tamagoyaki.find_unweighted_old"
...
class ButtonFindUnweighted(bpy.types.Operator):
    bl_idname = "tamagoyaki.find_unweighted"
...
class ButtonFixTooManyWeights(bpy.types.Operator):
    bl_idname      = "tamagoyaki.fix_toomanyweights"
...
class ButtonFreezeShape(bpy.types.Operator):
    bl_idname = "tamagoyaki.freeze_shape"
...
class ButtonConvertShapeToCustom(bpy.types.Operator):
    bl_idname = "tamagoyaki.convert_to_custom"
    bl_label = "Convert to Custom"
    bl_description = "Convert Selected Tamagoyaki Meshes to editable and fittable Custom Meshes (undo: CTRL-z)\n\nNotes:\nSelected Tamagoyaki Meshes are copied\nTamagoyaki Originals are Deleted\nSliders are attached\nShape keys are removed"
    bl_options = {'REGISTER', 'UNDO'}
...
class ButtonApplyShapeSliders(bpy.types.Operator):
    bl_idname = "tamagoyaki.apply_shape_sliders"
    bl_label = "Detach"
...
class CleanupCustomProps(bpy.types.Operator):
    bl_idname      = "tamagoyaki.cleanup_custom_props"
    bl_label       = "Cleanup"
    bl_description = '''Remove Tamagoyaki internal data.
This cleans up the meshes from all Tamagoyaki related information.
...
'''
    ...
    def execute(self, context):
        for ob in [ o for o in context.scene.objects if o.select_get()]:
            omode=util.ensure_mode_is("OBJECT")
            cleanupTamagoyakiData(ob, purge=True, restore=False)
    ...
    if freeze:
        ...
        for target in attached:
            ...
            if keep_tamagoyaki_properties and util.is_tamagoyaki_mesh(target):
                ...
                cleanupTamagoyakiData(target, purge, restore=False)
                log.debug("unbind from armature: cleaned tamagoyaki data from %s" % (target.name) )
...
def bind_to_armature(self,
    ...
    for target in attach_set:
        ...
        wcontext.active_weightmap = weights.get_active_weightmap(target)
        ...
        if not util.is_tamagoyaki_mesh(target):
...
class ButtonParentArmature(bpy.types.Operator):
    bl_idname = "tamagoyaki.parent_armature"
...
class ButtonRebindArmature(bpy.types.Operator):
    bl_idname = "tamagoyaki.rebind_armature"
    bl_label = "Rebind"
    bl_description = messages.tamagoyaki_reparent_armature_description_rebind
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def get_scope(context):
        ...
        scope = [o for o in scope if o.type=='MESH' and not 'tamagoyaki-mesh' in o]
    ...
    def execute_rebind(context, apply_as_bindshape):
        ...
        try:
            ...
            for child in scope:
                ...
                original_shape_dict = child.get('shape_buffer')
                current_shape_dict = shape.asDictionary(armobj, full=True)
                do_bind = apply_as_bindshape or not original_shape_dict
                if do_bind:
...
class ButtonReparentArmature(bpy.types.Operator):
    bl_idname = "tamagoyaki.reparent_armature"
    bl_label = "Rebind"
    bl_description = messages.tamagoyaki_reparent_armature_description_rebind
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        def get_target_armature(context, tamagoyakis):
            ob = context.object
            if ob and ob.type == 'ARMATURE' and 'tamagoyaki' in ob:
                arm_obj = ob
            elif tamagoyakis:
                arm_obj = tamagoyakis[0]
            else:
        ...
        try:
            arm_obj = get_target_armature(context, tamagoyakis)
            if not arm_obj:
...
class ButtonReparentMesh(bpy.types.Operator):
    ...
    bl_idname = "tamagoyaki.reparent_mesh"
    bl_label = "Rebind Mesh to Armature"
    bl_description = messages.tamagoyaki_reparent_mesh_description
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        target = context.scene.objects.get(self.object_name)
        if not target:
            log.warning("Could not find Mesh %s for rebind" % (target.name))
            return {'CANCELLED'}
...
class ButtonArmatureAllowStructureSelect(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_allow_structure_select"
...
class ButtonArmatureUnlockLocation(bpy.types.Operator):
    bl_idname      = "tamagoyaki.armature_unlock_loc"
...
class ButtonArmatureLockLocation(bpy.types.Operator):
    ...
    def execute(self, context):
        try:
            ...
            armature_lock_loc(context, op=self)
...
class ButtonArmatureResetPoseLSL(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_reset_pose_lsl"
...
class ButtonSupportInfo(bpy.types.Operator):
    bl_idname = "tamagoyaki.copy_support_info"
...
class ButtonDevkitManagerCutPreset(bpy.types.Operator):
    bl_idname = "tamagoyaki.devkit_manager_cut_preset"
...
class ButtonArmatureResetPose(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_reset_pose"
...
def create_armature_reset_action(context, subset):
    ...
    if subset == 'ANIMATED':
        animated_meshes = util.get_animated_meshes(context, arm, with_tamagoyaki=True, only_selected=False)
...
class ButtonArmatureApplyAsRestpose(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.ops.tamagoyaki.armature_bake(
...
class ButtonArmatureBake(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_bake"
    bl_label = "Apply as Restpose"
    bl_options = {'REGISTER', 'UNDO'}
    bl_description = messages.tamagoyaki_apply_as_restpose_text
...
Handle with Care!'''
    ...
    def execute(self, context):
        ...
        def apply_pose_as_restpose(arm_obj):
            ...
            bones = active.pose.bones
            for b in bones:
                bdata = b.bone.get('bind_mat')
                if not bdata:
                    log.warning("Bone %s has no bind mat" % b.name)
                    continue

                B = util.matrix_from_array(bdata)
                P = b.matrix.normalized()
                D = b.bone.matrix_local.normalized()
                X = Rz90I @ P @ D.inverted() @ Rz90 @ B
                
                bdata = util.matrix_to_array(X)
                b.bone['bind_mat'] = bdata
        ...
        def finalize_bound_meshes(context, arm_obj, selected_object_names, unselectable, invisible):
            ...
            if self.apply_armature_on_snap_rig:
                ...
                util.ensure_mode_is('OBJECT')
                util.object_select_set(arm_obj, True)
                bpy.ops.tamagoyaki.parent_armature(
                    bindSourceSelection = 'NONE',
        ...
        def prepare_workbench(context, arm_obj):
            ...
            log.info("| Backup current Shape to Scene")
        ...
        util.update_view_layer(context)
        bpy.ops.tamagoyaki.armature_jointpos_store(store_as_bind_pose=use_bind_pose)
...
class ArmatureSpineUnfoldUpper(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_spine_unfold_upper"
...
class ArmatureSpineDisplayUpper(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_spine_display_upper"
...
class ArmatureSpineDisplayLower(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_spine_display_lower"
...
class ArmatureSpineUnfoldLower(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_spine_unfold_lower"
...
class ArmatureSpineUnfold(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_spine_unfold"
    bl_label = "Unfold"
...
class ButtonAlphaMaskBake(bpy.types.Operator):
    bl_idname = "tamagoyaki.alphamask_bake"
    bl_label = "Bake Mask"
...
class ButtonDeformUpdate(bpy.types.Operator):
    bl_idname = "tamagoyaki.armature_deform_update"
...
class ButtonImportTamagoyakiShape(bpy.types.Operator):
    ...
    with_quads : BoolProperty(name='Use Quads',
        default = False,
        description = "If enabled: Create an Tamagoyaki with Quads, If disabled:Create a triangulated Tamagoyaki" )

    def draw(self, context):
        layout = self.layout
        obj = context.object
        if obj and util.object_select_get(obj) and obj.type == 'ARMATURE' and 'tamagoyaki' in obj:
            box = layout.box()
            col=box.column(align=True)
...
class ButtonExportDevkitConfiguration(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        config['%s - %s'%(brand,name)] = {
            ...
            'source.skeletontype' : props.SkeletonType,
            'target.rigtype'   : props.tgtRigType,
            'target.jointtype' : props.tgtJointType,
            'target.skeletontype' : props.tgtSkeletonType,
...
class ButtonImportDevkitConfiguration(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for section in config.sections():
            ...
            props.SkeletonType      = devkit.get('source.skeletontype')
            props.tgtRigType        = devkit.get('target.rigtype')
            props.tgtJointType      = devkit.get('target.jointtype')
            props.tgtSkeletonType   = devkit.get('target.skeletontype')
            ...
            try:
                bpy.ops.tamagoyaki.devkit_presets_update(name=section)
            except:
...
class ButtonExportSLCollada(bpy.types.Operator):
    ...
    @classmethod
    def description(cls, context, properties):
        if ButtonExportSLCollada.can_execute(context):
            return 'Export the selection as Collada-1.4.1 (.dae) with compatibility with SL, OpenSim and similar worlds'
        else:
            return "Collada export DISABLED: Please select one or more Meshes"

    @classmethod
    def poll(cls, context):
        return ButtonExportSLCollada.can_execute(context)
    ...
    def invoke(self, context, event):
        ...
        if selected_objects:
            ...
            help = get_help_page("EXPORT_WARN_M014")
            export_errors.append([msg, help])
            ...
            for obj in selected_objects:
                ...
                if arm is not None:
                    ...
                    if 'undeformable' in report['status']:
                        ...
                        help = get_help_page("EXPORT_WARN_M011")
                        export_errors.append([msg, help])
                    ...
                    if 'unweighted' in report['status']:
                        ...
                        help = get_help_page("EXPORT_WARN_M012")
                        export_errors.append([msg, help])

                    if 'zero_weights' in report['status']:
                        zero_weights = report['zero_weights']
                        msg = M013_msg_zero_verts % (len(zero_weights), obj.name)
                        help = get_help_page("EXPORT_WARN_M013")
                        export_errors.append([msg, help])
    ...
    def execute(self, context):
        ...
        try:
            print("+===================================================")
            print("| Tamagoyaki: Collada export to file [",self.filepath,"]")
            use_bind_pose = False
            export_restpose = False
            export_lsl = False
            armobjs = util.get_armatures(context.selected_objects)
            if len(armobjs) > 0:
                print("| Tamagoyaki: Found %d armatures" % len(armobjs))
            ...
            if armobj:
                hidden_objects =util.get_select_and_hide([armobj], hide_viewport=False)
            ...
            if good:
                ...
                print("| Tamagoyaki: Export returned with errors(See log)")
                status = {'CANCELLED'}
        except Exception as e:
            util.ErrorDialog.exception(e)
            status = {'CANCELLED'}
            ...
            if hidden_objects:
                util.set_select_and_hide(context, hidden_objects)
...
class DummyOp(bpy.types.Operator):
    ...
    bl_label = "-"
    bl_description = "Please Update this Armature before usage.\nSee N-Panel > Tamagoyaki > Rig Inspector > Rig Update Tool"

    def execute(self, context):
        return{'FINISHED'}

class UpdateTamagoyakiPopup(bpy.types.Operator):
    """Tooltip"""
    bl_idname = "tamagoyaki.update_tamagoyaki"
    bl_label = "Tamagoyaki Rig Version Mismatch"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def draw(self, context):
        obj     = context.object
        if not obj:
            print("UpdateTamagoyakiPopup: No Context Object set (aborting)")
            return

        layout = self.layout
        armatures = util.getTamagoyakiArmaturesInScene(context=context)

        if len(armatures) == 0:
            print("Tamagoyaki popup: No Armature found for %s" % (context.object.name) )
            return
        ...
        for armobj in armatures:
            ...
            if update:
                col_stat.column().operator("tamagoyaki.dop", text='',icon=icon, emboss=False)
            else:
        ...
        col.label(text="editing in this blend file. The Tamagoyaki Rig Update Tool is here:")
        col.label(text="")
        col.label(text="  N-Panel -> Tamagoyaki -> Rig Inspector -> Rig Update Tool")
        col.label(text="")
...
class BakeShapeOperator(bpy.types.Operator):
    bl_idname      = "tamagoyaki.bake_shape"
    bl_label       = "Bake Shape"
...
class BakeCleanupMaterialsOperator(bpy.types.Operator):
    bl_idname      = "tamagoyaki.material_bake_cleanup"
...
class AVAMaterial:
    ...
    def get_texture(self):
        texture = self.get_node_by_name("%s_tex" % self.material)
        if texture:
...
class BakeMaterialsOperator(bpy.types.Operator):
    bl_idname      = "tamagoyaki.material_bake"
...
def get_extended_weights(context, armobj, tgt, part, vert_mapping='TOPOLOGY'):
    ...
    if extended_src:
        ...
        print("Create Rig                : Transfered %d Weight Maps from Tamagoyaki template %s" % (len(extended_src.vertex_groups), extended_src.name ))
    ...
    for image in mat_images:
        ...
        if image.is_dirty or image.packed_file or image.source == 'GENERATED':
            ...
            if image.filepath_raw is None or "":
                ...
                msg = "> " + messages.M006_image_not_found + "\n"% (
                        util.shorten_text(image.name),
                        util.shorten_text(material.name)
                    )
                help = get_help_page("EXPORT_WARN_M006")
                warnings.append([msg, help])
                image_on_disk = False
            else:
                ...
                if source == dest:
                    ...
                    try:
                        ...
                        msg = "> " + messages.M007_image_copy_failed + ":\n"
                        help = get_help_page("EXPORT_WARN_M007")
                        warnings.append([msg, help])
...
def exportCollada(context,
    ...
    log_export.info("Export Tamagoyaki Collada for Target system[%s]" % (target_system) )
    ...
    for index, obj in enumerate(selected_objects):
        ...
        if export_armature:
            ...
            if arm is not None and arm not in armatures:
                if 'tamagoyaki' in arm:
                    armatures.append(arm)
                else:
                    msg = ("> " + M008_not_an_tamagoyaki + "\n") % (
    ...
    if weld_normals:
        ...
        try:
            ...
            log.error("aligning failed")

    else:
        log.warning("Export Aligned Normals is disabled")
        adjusted_normals = None
    #
    ...
    for arm in armatures:
        ...
        print("| Skeleton type       : %s" % arm.RigProp.SkeletonType )
    ...
    for index, obj_name in (enumerated_objects):
        ...
        if arm is not None:
            ...
            vcounter = 0
            ignored_groups = {}
            broken_vgroup_count = 0
            vgroup_max_index = len(meshobj.vertex_groups)-1
            for v in mesh_data_copy.vertices:
                vcounter += 1
                if vcounter % 1000 == 0:
                    progress += 1
                ...
                for g in v.groups:
                    ...
                    index = g.group
                    if index < 0 or index > vgroup_max_index:
                        broken_vgroup_count += 1
                        continue
    ...
    try:
        ...
        msg = "The file %s could not be written to the specified location.|\n" \
    ...
    if in_export_hierarchy:#bone.use_deform or not only_deform:
        ...
        if in_export_hierarchy:
            ...
            if use_blender: #export with blender profile
                ...
                dbone=pbone.bone
                if with_roll:
                    mat  = pbone.matrix #PoseBoneMatrix( obArm, pb)
                    axis, roll = dbone.AxisRollFromMatrix(mat.to_3x3())
                    if not util.isclose(roll,0):
                        subx(tech, 'roll', text="%f"%roll)

                if True or not Skeleton.has_connected_children(bone):
                    tail = dbone.tail_local - dbone.head_local
                    if with_sl_rot:
...
class TamagoyakiSnapToMesh(bpy.types.Operator):
    bl_idname = "tamagoyaki.snap_to_mesh"
...
on another mesh, it is not snapped'''
    ...
    def draw(self, context):
        ...
        if self.snapped != None or self.out_of_range != None:
            col.separator()
            box = col.box()
            col = box.column(align=True)
...
def create_visual_copy(context, obj):
    visual_copy           = visualCopyMesh(context, obj, apply_pose = False)
    visual_copy.data.name += "(frozen)"
    return visual_copy

def destroy_visual_copy(context, obj):
    util.unlink_object(context, obj)
    bpy.data.objects.remove(obj)


def get_world_co(verts, obj):
    for vert in verts:
        co = obj.matrix_world @ (vert.co)

        vert.co = co

def get_boundary_verts(bmsrc, context, obj, exportRendertypeSelection, filter_selected_verts):
    all_verts = not filter_selected_verts
    target_copy = create_visual_copy(context, obj)
    bmsrc.from_mesh(target_copy.data)
    verts = [vert for vert in bmsrc.verts if vert.is_boundary and (all_verts or vert.select)]
    get_world_co(verts, obj)
    destroy_visual_copy(context, target_copy)

    return verts

def get_all_verts(bmsrc, context, obj, exportRendertypeSelection, filter_selected_verts):
    all_verts = not filter_selected_verts
    target_copy = create_visual_copy(context, obj)
    bmsrc.from_mesh(target_copy.data)
    verts = [vert for vert in bmsrc.verts if all_verts or vert.select]
    get_world_co(verts, obj)
    destroy_visual_copy(context, target_copy)
    return verts
...
class WeldNormals(bpy.types.Operator):
    ...
    if has_normals:
        ...
        remark = "* This operator needs Blender 2.75 or newer\n"

    bl_description = "Align Normals to adjacent objects\nNotes:\n%s* When in edit mode, then only selected verts are processed" % (remark)
    ...
    def draw_generic(context, layout):
        col=layout.column(align=True)
        props = bpy.context.scene.SceneProp
        col.operator(WeldNormals.bl_idname, text="Align Selected")
        col.prop(props, "spk_filter_boundary_verts", text='Only Boundaries')
        col.prop(props, "spk_filter_smooth_faces", text='Only Smooth Faces')

    def draw(self, context):
        col=self.layout.column(align=True)
        col.prop(self, "spk_filter_boundary_verts", text='Only Boundaries')
        col.prop(self, "spk_filter_smooth_faces", text='Only Smooth Faces')

    def invoke(self, context, event):
        props = bpy.context.scene.SceneProp
        self.spk_filter_boundary_verts = props.spk_filter_boundary_verts
        self.spk_filter_smooth_faces = props.spk_filter_smooth_faces
        return self.execute(context)

    def execute(self, context):

        mobs = [ob for ob in context.visible_objects if util.object_visible_get(ob, context) and ob.type=="MESH"]


        active_ob = bpy.context.object
        active_ob_mode = util.set_object_mode('EDIT')

        adjusted_normals = self.get_adjusted_vertex_normals(context, mobs, 'PREVIEW', active_ob_mode)
        if adjusted_normals:
            for tgt in [obj for obj in mobs if obj.select_get() and obj.name in adjusted_normals]:
                ...
                for key, n in welded_normals.items():
                    ...
                    normals[3*key+1] = n[1]
                    normals[3*key+2] = n[2]
                original_mode = util.set_object_mode('OBJECT')

                tnormals = tuple(zip(*(iter(normals),) * 3))
                mesh.normals_split_custom_set(tnormals)

                util.set_object_mode(original_mode)


        util.set_active_object(context, active_ob)
        util.set_object_mode(active_ob_mode)

        return {'FINISHED'}

    def get_adjusted_vertex_normals(self, context, sources, exportRendertypeSelection, active_ob_mode):
        all_faces = not self.spk_filter_smooth_faces
        only_selected_verts = active_ob_mode=='EDIT'
        only_boundary_verts = self.spk_filter_boundary_verts

        def smooth_or_any(face):
            if all_faces:
                return True
            return face.smooth

        def get_verts(bm, obj):
            if only_boundary_verts:
                target_verts = get_boundary_verts(bm, context, obj, exportRendertypeSelection, only_selected_verts)
            else:
                target_verts = get_all_verts(bm, context, obj, exportRendertypeSelection, only_selected_verts)
        ...
        for obj in [obj for obj in sources if util.object_select_get(obj)]:


            target_verts = get_verts(bm_target, obj)
            ...
            for otherobj in [ob for ob in sources if ob != obj]:
                ...
                source_verts = get_verts(bm_source, otherobj)
                ...
                for target_vert in [v for v in target_verts if any([e.smooth for e in v.link_edges])]:
                    ...
                    if source_vert:
                        ...
                        avnor = Vector((0,0,0))
                        for face in [f for f in source_vert.link_faces if smooth_or_any(f)]:
                            avnor += face.normal
                        for face in [f for f in target_vert.link_faces if smooth_or_any(f)]:
                             avnor += face.normal
                        avnor.normalize()
                        for loop in [l for l in target_vert.link_loops if smooth_or_any(l.face)]:
                            normals[loop.index] = avnor
                            log.warn("adding normal for vertex %d %s" % (loop.vert.index, loop.vert.select))
